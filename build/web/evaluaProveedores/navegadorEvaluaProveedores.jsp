...
<html>
    <head>
        ...
        <script>
            ...
            window.location = 'navegadorPeriodosEvalucionProveedor.jsf';
      }
...
%>
        ...
        fecha_final=fecha_finalV[2]+"/"+fecha_finalV[1]+"/"+fecha_finalV[0];
            ...
            while (rs.next()) {
                ...
                int codCategoriaProveedor = rs.getInt(6);
                double puntaje = redondear(rs.getDouble(7),2);
                ...
                String sql_oc="select top 1 i.COD_ORDEN_COMPRA,o.NRO_ORDEN_COMPRA,i.FECHA_INGRESO_ALMACEN from INGRESOS_ALMACEN i , INGRESOS_ALMACEN_DETALLE id,ORDENES_COMPRA o";
                ...
                int cod_oc = 0;
                while(rs_oc.next()){
                    nro_oc=rs_oc.getInt(2);
                    Date fee=rs_oc.getDate(3);
                    fechaIngreso=f.format(fee);
                    cod_oc = rs_oc.getInt("cod_orden_compra");
                }
...
%>
                <tr  class="border" title=" " >
                    <td height="30px" align="left" style="border-left : solid #333333 1px;border-bottom: solid #333333 1px;">
                        ...
                        <input type="hidden" name="codOrdenCompra" value="<%=cod_oc%>"/>
                        &nbsp;<%=cont%>&nbsp;
                    </td>
                    ...
                    <td  align="right" style="border-left : solid #333333 1px;border-bottom: solid #333333 1px;"><%=fechaIngreso%>&nbsp; </td>
                    <%--td  align="center" style="border-left : solid #333333 1px;border-bottom: solid #333333 1px;"><a href="navegadorEvaluaProveedoresDetalle.jsf?codProveedor=<%=codProveedor%>&nomProveedor=<%=nombreProveedor%>&codMaterial=<%=codMaterial%>&nomMaterial=<%=nombreMaterial%>&cod_tipo_inc_regional=<%=codPeriodoEvaluacion%>" style="color:blue;" target="_BLANK" >&nbsp;Evaluar&nbsp;</a></td>
                    <td  align="center" style="border-left : solid #333333 1px;border-bottom: solid #333333 1px;border-right: solid #333333 1px;"><a href="navegadorEvaluaProveedoresImprimir.jsf?codProveedor=<%=codProveedor%>&nomProveedor=<%=nombreProveedor%>&codMaterial=<%=codMaterial%>&nomMaterial=<%=nombreMaterial%>&puntaje=<%=puntaje%>&cod_tipo_inc_regional=<%=codPeriodoEvaluacion%>" style="color:blue;" target="_BLANK">&nbsp;Imprimir&nbsp;</a></td--%>
...
%>
            ...
            %>
            <input type="button" value="Guardar" class="btn" onclick="guardarEvaluacionProveedor(<%=codPeriodoEvaluacion%>)"/>
