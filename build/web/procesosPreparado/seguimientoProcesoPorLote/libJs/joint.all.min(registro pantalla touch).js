  ...
  var xcd=0;
   function asignarPuntos(linea)
   {
        ...
        linea._opt.vertices[0]=fd(xcd,(start.Y+(start.H/2)));
        linea._opt.vertices[1]=fd(xcd,(end.Y+(end.H/2)));
        linea.update();
        xcd+=8;
   }
   function crunch(linea)
   {
        var end=linea._end.shape;
        var start=linea._start.shape;
        linea._opt.vertices[0]=fd((start.X+(start.W/2)),(end.Y+(end.H/2)));
        linea.update();
   }
var codProceso='';
var contmenos=0;
...
function mostrarSeguimiento()
            {
				...
				c.options.length=0;
				var cont=0;
				for(var i=0;i<personalSelect.length;i+=2)
				{

					c.options[cont]=new Option(personalSelect[i+1],personalSelect[i],"");
					if(procesoActual.properties.codPersonal==personalSelect[i])
					{
						c.selectedIndex=cont;
					}
                ...
                /*ajax.open("GET","ajaxRegistrarSeguimiento.jsp?&noCache="+ Math.random()+
                          "&codProceso="+codProceso+
                          "&codSubProceso="+codSubProceso+
                          "&codLote="+codLote+
                          "&personal="+personalSelect
                      ,true);
                ajax.onreadystatechange=function(){
                    if (ajax.readyState==4) {
                        div_mostrar_seguimiento.innerHTML=ajax.responseText;
                    }
                }
                codProcesoActual=codProceso;
                codSubProcesoActual=codSubProceso;
 ...
 */Raphael=function(){
    ...
    var t=D.Joint;D=t.dia.Element;var F=t.point,g=t.dia.uml={};t.arrows.aggregation=function(){
        ...
        getPersonal:function(g){
			...
			s=this.wrapper.getBBox();i=this.paper.text(s.x+i.actionsOffsetX,s.y+i.labelOffsetY+i.labelHeight+i.actionsOffsetY+n.coorYText,i.operario);
            k=i.getBBox();i.attr(n.textNormal);i.translate(0,k.height/2);
			n.coorYText+=k.height;
			g.addInner(i);
			i=this.properties;
		...
		updatePersonal:function(g,codigoPersonal,nombrePersonal,estadoConforme){
			...
			var i=this.properties;
			s=this.wrapper.getBBox();i=this.paper.text(s.x+i.actionsOffsetX,i.cooryOPerario,'-'+i.operario);
            k=i.getBBox();i.attr(n.textNormal);i.translate(0,k.height/2);
			g.addInner(i);
			i=this.properties;
			s=this.wrapper.getBBox();i=this.paper.text(s.x+i.actionsOffsetX,i.cooryOPerario+k.height,'CONFORME:'+(i.conforme=='1'?'SI':(i.conforme=='0'?'NO':'')));
            k=i.getBBox();i.attr(n.textBold);i.translate(0,k.height/2);
			g.addInner(i);
        },
