...
<%
            ...
            <br>
            <table  align="center" width="80%" class="tablaReporte" style="border : solid #f2f2f2 1px;" cellpadding="0" cellspacing="0">
                ...
                String consulta="select ppr.COD_PROGRAMA_PROD,appi.COD_ACTIVIDAD, sppi.HORAS_HOMBRE,appi.ORDEN,ap.NOMBRE_ACTIVIDAD,"+
                                  ...
                                  " order by ppr.COD_PROGRAMA_PROD,ae.NOMBRE_AREA_EMPRESA,appi.ORDEN,ppr.FECHA_INICIO,p.AP_PATERNO_PERSONAL,p.AP_MATERNO_PERSONAL,p.NOMBRES_PERSONAL asc";
                System.out.println("consulta detalle personal "+consulta);
                Statement st=con.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);
                ...
                SimpleDateFormat sdf=new SimpleDateFormat("dd/MM/yyyy HH:mm");
                        while(res.next())
                        {
                            if(codProgramaProdCabecera!=res.getInt("COD_PROGRAMA_PROD"))
                            {
                                if(codProgramaProdCabecera>0)
                                {
                                    ...
                                    out.println("<tr><th  class='outputText2' style='border : solid #D8D8D8 1px' rowspan='"+contProgProd+"' >"+nombreProgProg+"</th>"+innerHTMLArea+
                                                                                    ...
                                                                                    "<th rowspan='"+contCodActividad+"'>"+res.getInt("ORDEN")+"</th>"+
                                                                                    "<th rowspan='"+contCodActividad+"'>"+res.getString("NOMBRE_ACTIVIDAD")+"</th>" +
                                                                                    "<th rowspan='"+contCodActividad+"'>"+format.format(sumaHHactividad)+"</th>"+innerHTMLPersonal);
                                    out.println("<tr bgcolor='#FF8C00'><th colspan='8' align='right'><span style='font-weight:bold'>Total:</span></th><th><span style='font-weight:bold'>"+format.format(sumaHHPrograma)+"</span></th><tr>");
                                    res.next();
                                }
                            ...
                            if(codActividadCabecera!=res.getInt("COD_ACTIVIDAD"))
                            {
                                res.previous();
                                innerHTMLActividad+=(innerHTMLActividad.equals("")?"":"<tr>")+"<th rowspan='"+contCodActividad+"'>"+res.getInt("ORDEN")+"</th>"+
                                                                                              "<th rowspan='"+contCodActividad+"'>"+res.getString("NOMBRE_ACTIVIDAD")+"</th>" +
                                                                                              "<th rowspan='"+contCodActividad+"'>"+format.format(sumaHHactividad)+"</th>"+
                                                                                              innerHTMLPersonal;
                                res.next();
                                innerHTMLPersonal="";
                                codActividadCabecera=res.getInt("COD_ACTIVIDAD");
                            ...
                            innerHTMLPersonal+=(innerHTMLPersonal.equals("")?"":"<tr>")+"<th  class='bordeNegroTd' ><span class='outputText2'>"+res.getString("nombrePersonal")+"</span></th>"+
                                    "<th  class='bordeNegroTd' ><span class='outputText2'>"+sdf.format(res.getTimestamp("FECHA_INICIO"))+"</span></th>"+
                                    "<th  class='bordeNegroTd' ><span class='outputText2'>"+sdf.format(res.getTimestamp("FECHA_FINAL"))+"</span></th>" +
                                    "<th  class='bordeNegroTd' ><span class='outputText2'>"+res.getDouble("horas_hombre_personal")+"</span></th>"+
                                    "</tr>";
                            sumaHHactividad+=res.getDouble("horas_hombre_personal");
                            sumaHHPrograma+=res.getDouble("horas_hombre_personal");
                            contProgProd++;
                            contCodArea++;
                            contCodActividad++;
                        }
