...
<%
            ...
            String codLoteProduccionR = request.getParameter("codLoteProduccion");
            String codAreaEmpresaActv = request.getParameter("codAreaEmpresaActv");
            Connection con = null;
            ...
            consulta = " select nombre_area_empresa_cp,COD_LOTE_PRODUCCION,    sum(horas_hombre) horas_hombre,  sum(unidades_producidas) unidades_producidas," +
                    " sum(unidades_producidas_extra) unidades_producidas_extra, sum(horas_maquina) horas_maquina,nombre_prod_semiterminado," +
                    " nombre_area_empresa_actv,cod_area_empresa_actv,sum(horas_maquina_std) horas_maquina_std,sum(horas_hombre_std) horas_hombre_std " +
                    " from ( select ae.nombre_area_empresa nombre_area_empresa_cp, p.COD_LOTE_PRODUCCION,segui.horas_hombre,segui.unidades_producidas," +
                    " segui.unidades_producidas_extra,segui.horas_maquina, cp1.nombre_prod_semiterminado," +
                    ...
                    " inner join ACTIVIDADES_PRODUCCION a on a.COD_ACTIVIDAD = afm.COD_ACTIVIDAD and a.COD_ESTADO_REGISTRO = 1" +
                    " inner join personal pe on pe.COD_PERSONAL = s.COD_PERSONAL" +
                    " where s.FECHA_INICIO >= '2014/03/01 00:00:00' and s.FECHA_FINAL <= '2014/03/31 23:59:59'" +
                    " and s.COD_LOTE_PRODUCCION = p.COD_LOTE_PRODUCCION and s.COD_COMPPROD = p.COD_COMPPROD" +
                    " and s.COD_FORMULA_MAESTRA = p.COD_FORMULA_MAESTRA and s.COD_PROGRAMA_PROD = p.COD_PROGRAMA_PROD" +
                    " and s.COD_TIPO_PROGRAMA_PROD = p.COD_TIPO_PROGRAMA_PROD) segui" +
                    " where p.COD_TIPO_PROGRAMA_PROD = 1 and p.COD_LOTE_PRODUCCION" +
                    " in (select distinct sp.COD_LOTE_PRODUCCION from SEGUIMIENTO_PROGRAMA_PRODUCCION_PERSONAL sp" +
                    " where  sp.FECHA_INICIO >= '2014/03/10 00:00:00' and sp.FECHA_FINAL <= '2014/03/11 23:59:59') and segui.horas_hombre > 0" +
                    " and afm1.COD_AREA_EMPRESA in(96, 84, 102, 1003,97,76))as tabla" +
                    " group by nombre_area_empresa_cp,COD_LOTE_PRODUCCION,nombre_prod_semiterminado,nombre_area_empresa_actv,cod_area_empresa_actv" +
                    "  order by cod_lote_produccion,case when COD_AREA_EMPRESA_actv = 76 then 1" +
                    "  when COD_AREA_EMPRESA_actv = 96 then 2" +
                    "  when COD_AREA_EMPRESA_actv = 97 then 3" +
                    "  when COD_AREA_EMPRESA_actv = 84 then 4  else 100 end ";
                    //" order by cod_lote_produccion ";
            consulta = " select nombre_area_empresa_cp,  COD_LOTE_PRODUCCION,  sum(horas_hombre) horas_hombre,  sum(unidades_producidas) unidades_producidas," +
                    " sum(unidades_producidas_extra) unidades_producidas_extra,    sum(horas_maquina) horas_maquina,   nombre_prod_semiterminado," +
                    " sum(horas_maquina_std) horas_maquina_std,  sum(horas_hombre_std) horas_hombre_std,  cod_compprod,   cod_formula_maestra," +
                    " cod_programa_prod,       nombre_actividad,cod_actividad_formula,orden_actividad,nombre_area_empresa_actv" +
                    " from ( select ae.nombre_area_empresa nombre_area_empresa_cp, p.COD_LOTE_PRODUCCION,  segui.horas_hombre,   segui.unidades_producidas," +
                    " segui.unidades_producidas_extra,segui.horas_maquina, cp1.nombre_prod_semiterminado, ae1.NOMBRE_AREA_EMPRESA nombre_area_empresa_actv," +
                    " ae1.cod_area_empresa cod_area_empresa_actv, maf.HORAS_HOMBRE horas_hombre_std, maf.HORAS_MAQUINA horas_maquina_std," +
                    " apr.NOMBRE_ACTIVIDAD, afm1.COD_ACTIVIDAD_FORMULA, afm1.orden_actividad," +
                    " cp1.cod_compprod, f.cod_formula_maestra, p.cod_programa_prod" +
                    " from programa_produccion p" +
                    " inner join componentes_prod cp1 on cp1.cod_compprod = p.cod_compprod" +
                    " inner join areas_empresa ae on ae.cod_area_empresa = cp1.cod_area_empresa" +
                    " inner join FORMULA_MAESTRA f on f.COD_COMPPROD = cp1.COD_COMPPROD and f.COD_ESTADO_REGISTRO = 1" +
                    ...
                    " sum(sppr.HORAS_MAQUINA) horas_maquina" +
                    " from SEGUIMIENTO_PROGRAMA_PRODUCCION_PERSONAL s" +
                    " inner join SEGUIMIENTO_PROGRAMA_PRODUCCION sppr on sppr.COD_COMPPROD = s.COD_COMPPROD and sppr.COD_FORMULA_MAESTRA = s.COD_FORMULA_MAESTRA and" +
                    " sppr.COD_LOTE_PRODUCCION = s.COD_LOTE_PRODUCCION and sppr.COD_TIPO_PROGRAMA_PROD = s.COD_TIPO_PROGRAMA_PROD and sppr.COD_PROGRAMA_PROD = s.COD_PROGRAMA_PROD and" +
                    " sppr.COD_ACTIVIDAD_PROGRAMA = s.COD_ACTIVIDAD_PROGRAMA" +
                    " inner join ACTIVIDADES_FORMULA_MAESTRA afm on afm.COD_ACTIVIDAD_FORMULA = sppr.COD_ACTIVIDAD_PROGRAMA and afm.COD_FORMULA_MAESTRA = p.COD_FORMULA_MAESTRA and" +
                    " afm.COD_ESTADO_REGISTRO = 1 and afm.COD_ACTIVIDAD_FORMULA = afm1.COD_ACTIVIDAD_FORMULA" +
                    " inner join ACTIVIDADES_PRODUCCION a on a.COD_ACTIVIDAD = afm.COD_ACTIVIDAD and a.COD_ESTADO_REGISTRO = 1" +
                    " inner join personal pe on pe.COD_PERSONAL = s.COD_PERSONAL" +
                    " where s.FECHA_INICIO >= '2014/03/01 00:00:00' and s.FECHA_FINAL <= '2014/03/31 23:59:59' and s.COD_LOTE_PRODUCCION = p.COD_LOTE_PRODUCCION and" +
                    " s.COD_COMPPROD = p.COD_COMPPROD and s.COD_FORMULA_MAESTRA = p.COD_FORMULA_MAESTRA and s.COD_PROGRAMA_PROD = p.COD_PROGRAMA_PROD and" +
                    " s.COD_TIPO_PROGRAMA_PROD = p.COD_TIPO_PROGRAMA_PROD) segui" +
                    " where p.COD_TIPO_PROGRAMA_PROD = 1 and p.COD_LOTE_PRODUCCION in (select distinct sp.COD_LOTE_PRODUCCION" +
                    " from SEGUIMIENTO_PROGRAMA_PRODUCCION_PERSONAL sp where sp.FECHA_INICIO >='2014/03/10 00:00:00' and sp.FECHA_FINAL <= '2014/03/11 23:59:59') and" +
                    " segui.horas_hombre > 0 and afm1.COD_AREA_EMPRESA in (96, 84, 102, 1003, 97, 76)" +
                    " and p.COD_LOTE_PRODUCCION = '"+codLoteProduccionR+"' and p.COD_PROGRAMA_PROD = '"+codProgramaProd+"' and p.COD_COMPPROD = '"+codCompProd+"' and p.COD_FORMULA_MAESTRA = '"+codFormulaMaestra+"' and ae1.cod_area_empresa= '"+codAreaEmpresaActv+"') as tabla" + //and ae1.cod_area_empresa = '"+codAreaEmpresaActv+"'
                    " group by nombre_area_empresa_cp,COD_LOTE_PRODUCCION, nombre_prod_semiterminado, cod_area_empresa_actv ,nombre_area_empresa_actv, nombre_actividad,cod_actividad_formula, cod_compprod, cod_formula_maestra, cod_programa_prod, cod_lote_produccion,orden_actividad" +
                    " order by case when COD_AREA_EMPRESA_actv = 76 then 1" +
                    "  when COD_AREA_EMPRESA_actv = 97 then 2" +
                    "  when COD_AREA_EMPRESA_actv = 96 then 3" +
                    "  when COD_AREA_EMPRESA_actv = 84 then 4  else 100 end asc,orden_actividad ";
            System.out.println("consulta " + consulta);
            //con = null;
            //con = Util.openConnection(con);
...
//                i++;
            ...
            double horasHombreStd = 0.0;
            double horasMaquinaStd = 0.0;
            while(ii.hasNext()){
                ...
                if(!codLoteProduccion.equals(f.get("codLoteProduccion").toString()) || !nombreAreaEmpresaCp.equals(f.get("nombreAreaEmpresaCp").toString()) || !nombreProdSemiterminado.equals(f.get("nombreProdSemiterminado").toString())){
                    ...
                    mensajeCorreo+= " <td  align='center' style='border : solid #f2f2f2 1px;' rowspan='"+this.cantidadFilas(f.get("nombreProdSemiterminado").toString(), f.get("codLoteProduccion").toString(),f.get("nombreAreaEmpresaCp").toString(), reportes)+"' >"+f.get("codLoteProduccion").toString()+"</td>";
                ...
                mensajeCorreo+= " <td  align='center' style='border : solid #f2f2f2 1px;' >"+formato.format(f.get("horasHombre"))+"</td>"+
                    ...
                    "</tr>";
                horasHombre = horasHombre + Double.valueOf(f.get("horasHombre").toString());
                horasMaquina = horasMaquina + Double.valueOf(f.get("horasMaquina").toString());
                horasHombreStd = horasHombreStd + Double.valueOf(f.get("horasHombreStd").toString());
                horasMaquinaStd = horasMaquinaStd + Double.valueOf(f.get("horasMaquinaStd").toString());
            }
            mensajeCorreo +="<tr><td></td><td></td><td align='center' style='border : solid #f2f2f2 1px;'>"+formato.format(horasHombre)+"</td><td align='center' style='border : solid #f2f2f2 1px;'>"+formato.format(horasMaquina)+"</td><td></td><td></td><td></td><td></td><td style='border : solid #f2f2f2 1px;'>"+formato.format(horasHombreStd)+"</td><td style='border : solid #f2f2f2 1px;'>"+formato.format(horasMaquinaStd)+"</td></tr>  " +
                    "</table><br/><br/>Sistema Atlas";
