...
String consulta = " select (CAST(pp.COD_PROGRAMA_PROD as varchar)+' '+CAST(pp.COD_COMPPROD as varchar)+' '+CAST(pp.COD_FORMULA_MAESTRA as varchar)"+
                  " +' '+CAST(pp.COD_TIPO_PROGRAMA_PROD AS VARCHAR)) AS CODIGOS,pp.COD_LOTE_PRODUCCION,cp.nombre_prod_semiterminado,tpp.NOMBRE_TIPO_PROGRAMA_PROD,isnull(tcf.valor_tipo_compprod_formato,0) valor_tipo_compprod_formato"+
                  ...
                  " COMPONENTES_PROD cp on cp.COD_COMPPROD=pp.COD_COMPPROD left outer join tipos_compprod_formato tcf on tcf.cod_tipo_compprod_formato = cp.cod_tipo_compprod_formato "+
                  ...
                  " from PROGRAMA_PRODUCCION_CRONOGRAMA ppc where ppc.COD_ESTADO_PROGRAMA_PRODUCCION_CRONOGRAMA in (1,3)) ORDER BY valor_tipo_compprod_formato desc";
...
out.println(" <tr class='headerClassACliente'><td></td> <td height='35px'>"+
            ...
            "<td>TIPO FORMATO TABLETEADO</td></tr>");
while(rs.next())
{
    ...
    out.println("<td>"+rs.getString("VALOR_TIPO_COMPPROD_FORMATO")+"</td>");
    out.println("</tr>");
}
out.println("</table>");
rs.close();
