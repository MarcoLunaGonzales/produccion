        ...
        while (i.hasNext()){
            ...
            String sql  = " SELECT top 1 (select c.nombre_prod_semiterminado from COMPONENTES_PROD c where c.COD_COMPPROD = s.COD_PROD),s.COD_PROD";                    
                   ...
                   sql = " select distinct s.COD_PROD,cp.nombre_prod_semiterminado,fm.cod_formula_maestra," +
                         ...
                         ",ppr.cod_tipo_programa_prod from SALIDAS_ALMACEN s inner join COMPONENTES_PROD cp on cp.COD_COMPPROD = s.COD_PROD" +
            ...
            int codTipoProgramaProd = rs.getInt("cod_tipo_programa_prod");
                %>
                        <tr>
                            ...
                            <td class=colh align="center" style="border : solid #f2f2f2 1px;" colspan="6"><b><%=cantidadLote%>  </b> </td>
                        </tr>
                <%
                String consulta = " select distinct fm.COD_FORMULA_MAESTRA,m.NOMBRE_MATERIAL,ROUND(fmp.CANTIDAD,2),um.abreviatura,um.NOMBRE_UNIDAD_MEDIDA,m.cod_material, fmp.nro_preparaciones,m.cod_grupo,e.nombre_estado_registro," +
                ...
                consulta = "select nombre_material,abreviatura,sum(salidas_material) salidas_material,sum(devoluciones_material) devoluciones_material,nombre_material_fm,sum(cantidad_fm) cantidad_fm,abreviatura_fm,sum(cantidad_fm1) cantidad_fm1 from (" +
                        ...
                        " full outer join formula_maestra_detalle_ep fmdmp on fmdmp.cod_formula_maestra = fm.cod_formula_maestra and fmdmp.cod_material = m.cod_material and fmdmp.COD_PRESENTACION_PRIMARIA = prp.COD_PRESENTACION_PRIMARIA" +
                        " full outer join materiales m2 on m2.cod_material = fmdmp.cod_material left outer join unidades_medida um2 on um2.cod_unidad_medida = fmdmp.cod_unidad_medida " +
                        " where sa1.COD_LOTE_PRODUCCION = '"+bean.getCodLoteProduccion()+"' and sa1.COD_PROD = '"+codCompProd+"'" +
                        " and m.COD_MATERIAL in (select m1.COD_MATERIAL from MATERIALES m1,grupos g" +
                ...
                while(rs2.next()){
                    ...
                    String abreviaturaFM = rs2.getString("abreviatura_fm")==null?"":rs2.getString("abreviatura_fm");
                    double diferencia = Math.abs((salidasMaterial-devolucionesMaterial) - cantidadFM);
                    double variacion = cantidadFM *Double.valueOf(porcentajeVariacion)/100;
                    String estilo = diferencia>=variacion?"background-color:red":"";
                    %>
                ...
                consulta = "select nombre_material,abreviatura,sum(salidas_material) salidas_material,sum(devoluciones_material) devoluciones_material,nombre_material_fm,sum(cantidad_fm) cantidad_fm,abreviatura_fm,cod_presentacion_producto,sum(cantidad_fm1) cantidad_fm1,NOMBRE_PRODUCTO_PRESENTACION from (" +
                        " select distinct m.nombre_material,um.abreviatura, (select sum(sd.CANTIDAD_SALIDA_ALMACEN) salidas" +
                        " from SALIDAS_ALMACEN_DETALLE sd" +
                        " inner join SALIDAS_ALMACEN s on s.COD_SALIDA_ALMACEN = sd.COD_SALIDA_ALMACEN" +
                        " where sd.COD_MATERIAL = m.cod_material and s.COD_LOTE_PRODUCCION = sa1.COD_LOTE_PRODUCCION and s.COD_ESTADO_SALIDA_ALMACEN = 1" +
                        ...
                        " and sa.cod_almacen = '1' and d.cod_almacen = '1' and c.COD_CAPITULO = 2" +
                        " and ia.cod_estado_ingreso_almacen = 1 and d.cod_estado_devolucion = 1" +
                        " and sa.COD_TIPO_SALIDA_ALMACEN in (1, 2, 4, 5, 6, 8) and m1.cod_material = m.cod_material and sa.COD_PRESENTACION = sa1.COD_PRESENTACION) devoluciones_material,m2.nombre_material nombre_material_fm,fmdmp.cantidad cantidad_fm,um2.abreviatura abreviatura_fm,fmdmp.cod_presentacion_producto,0 cantidad_fm1,prp.NOMBRE_PRODUCTO_PRESENTACION" +
                        " from SALIDAS_ALMACEN sa1" +
                        " inner join SALIDAS_ALMACEN_DETALLE sad on sad.COD_SALIDA_ALMACEN = sa1.COD_SALIDA_ALMACEN" +
                        " inner join materiales m on m.COD_MATERIAL = sad.COD_MATERIAL" +
                        " inner join unidades_medida um on um.cod_unidad_medida = m.cod_unidad_medida " +
                        ...
                        " inner join PRESENTACIONES_PRODUCTO prp on prp.cod_presentacion = cprp.COD_PRESENTACION " + //and cprp.COD_PRESENTACION = sa1.COD_PRESENTACION and cprp.COD_TIPO_PROGRAMA_PROD = ppr.COD_TIPO_PROGRAMA_PROD
                        " left outer join formula_maestra_detalle_es fmdmp on fmdmp.cod_formula_maestra = fm.cod_formula_maestra and fmdmp.cod_material = m.cod_material and fmdmp.COD_PRESENTACION_PRODUCTO = cprp.COD_PRESENTACION" + //and fmdmp.cod_tipo_programa_prod = ppr.cod_tipo_programa_prod
                        ...
                        " inner join FORMULA_MAESTRA fm on fm.COD_COMPPROD = cp.COD_COMPPROD" +
                        " inner join FORMULA_MAESTRA_DETALLE_ES fmdmp on fmdmp.COD_FORMULA_MAESTRA = fm.COD_FORMULA_MAESTRA and fmdmp.cod_tipo_programa_prod = ppr.cod_tipo_programa_prod and fmdmp.COD_PRESENTACION_PRODUCTO = cprp.COD_PRESENTACION" +
                        " inner join materiales m on m.COD_MATERIAL = fmdmp.COD_MATERIAL" +
                        " inner join UNIDADES_MEDIDA um on um.COD_UNIDAD_MEDIDA = fmdmp.COD_UNIDAD_MEDIDA" +
                        " inner join PRESENTACIONES_PRODUCTO prp on prp.cod_presentacion = fmdmp.COD_PRESENTACION_PRODUCTO " +
                        " where fm.COD_COMPPROD = '"+codCompProd+"' and ppr.cod_tipo_programa_prod = '"+codTipoProgramaProd+"' ) as tabla group by nombre_material,abreviatura,nombre_material_fm,abreviatura_fm,cod_presentacion_producto,nombre_producto_presentacion" +
                ...
                while(rs3.next()){
                    ...
                    if(rs3.getInt("cod_presentacion_producto")>0 || rs3.getString("nombre_material_fm")==null){
                    %>
                    <tr style="<%=estilo%>">
