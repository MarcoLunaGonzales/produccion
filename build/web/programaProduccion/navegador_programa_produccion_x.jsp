        ...
        <%--<body onLoad="window.defaultStatus='Hola, yo soy la barra de estado.';">--%>
            ...
            <a4j:form id="form1">
                <div align="center">
                    ...
                    <a4j:commandButton value="Eliminar"    styleClass="btn"  action="#{ManagedProgramaProduccion.eliminarProgProd}" onclick="if(confirm('Esta seguro de eliminar el registro?')==false){return false}" oncomplete="if(#{ManagedProgramaProduccion.mensaje!=''}){alert('#{ManagedProgramaProduccion.mensaje}')}else{location='navegador_programa_produccion.jsf'}" /><%-- else{return eliminar('form1:dataFormula');} --%>
            ...
            <rich:modalPanel id="panelIngresoAcondicionamiento"
                        ...
                        oncomplete="if(#{ManagedProgramaProduccion.mensajes!=''}){alert('#{ManagedProgramaProduccion.mensajes}');if(#{ManagedProgramaProduccion.mensajeSolicitudAutomatica!=''}){alert('#{ManagedProgramaProduccion.mensajeSolicitudAutomatica}')}}" /> <%-- oncomplete="if(#{ManagedProgramaProduccion.mensajes!=''}){alert('#{ManagedProgramaProduccion.mensajes}');}" --%>
                        ...
                        <a4j:commandButton  value="Entrega Total XXXXXX" styleClass="btn"
                        onclick="if(confirm('Esta seguro de guardar esta informacion')==false){return false;}else{javascript:Richfaces.hideModalPanel('panelIngresoAcondicionamiento')}"
                        actionListener="#{ManagedProgramaProduccion.guardarIngresosAcondicionamiento_action}" reRender="contenidoProgramaProduccion" ajaxSingle="false" status="statusPeticion"
<<<<<<< .mine
                        oncomplete="if(#{ManagedProgramaProduccion.mensajes!=''}){alert('#{ManagedProgramaProduccion.mensajes}');
                        var a = '#{ManagedProgramaProduccion.mensajes}';
                        /*alert(a+'value');*/
                        if(a.substring(0,60)=='Termino de registrar correctamente con el numero de ingreso '){
                            if(confirm('desea registrar devoluciones?')==true){agregarDevolucion();}
                        }
                        }else{alert('Termino de registrar el ingreso correctamente');if(confirm('desea registrar devoluciones?')==true){agregarDevolucion();}}"
                        />
                        ...
                        <%-- oncomplete="if(#{ManagedProgramaProduccion.mensajes!=''}){alert('#{ManagedProgramaProduccion.mensajes}');}else{alert('Termino de registrar el ingreso correctamente');if(confirm('desea registrar devoluciones?')==true){agregarDevolucion();}}" --%>
=======
                        oncomplete="agregarDevolucion();if(#{ManagedProgramaProduccion.mensajes!=''}){alert('#{ManagedProgramaProduccion.mensajes}');if(#{ManagedProgramaProduccion.mensajeSolicitudAutomatica!=''}){alert('#{ManagedProgramaProduccion.mensajeSolicitudAutomatica}')}}" /><%-- oncomplete="if(#{ManagedProgramaProduccion.mensajes!=''}){alert('#{ManagedProgramaProduccion.mensajes}');}else{alert('Termino de registrar el ingreso correctamente');if(confirm('desea registrar devoluciones?')==true){agregarDevolucion();}}" --%>
>>>>>>> .r78
