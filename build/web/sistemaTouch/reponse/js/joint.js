...
function mostrarSeguimiento()
            {
				...
				document.getElementById('divDetalle').style.marginTop=((window.scrollY+30)+'px');
                ...
                d.options.length=0;
				var cont=0;
                c.options[cont]=new Option("No Registrado","0","true");
                d.options[cont]=new Option("No Registrado","0","true");
                cont++;
                for(var i=0;i<personalSelect.length;i+=2)
				{
					c.options[cont]=new Option(personalSelect[i+1],personalSelect[i],"");
                    d.options[cont]=new Option(personalSelect[i+1],personalSelect[i],"");
					cont++;
				}
    ...
    var t=D.Joint;D=t.dia.Element;var F=t.point,g=t.dia.uml={};t.arrows.aggregation=function(){
        ...
        getMateriales:function(g){
			...
			var	s=this.wrapper.getBBox();
                i=this.paper.text(s.x+i.actionsOffsetX,s.y+i.labelOffsetY+i.labelHeight+i.actionsOffsetY,'MATERIALES:\n');
                k=i.getBBox();i.attr(n.textBold);i.translate(0,k.height/2);
				n.coorYText+=k.height;
				g.addInner(i);
			i=this.properties;
			var k='';
            for(var y=0;y<i.materiales.length;y+=2)
                k+='-'+i.materiales[y]+": "+i.materiales[y+1]+"\n";
                k=k.replace(/^\s\s*/,"").replace(/\s\s*$/,"");
                s=this.wrapper.getBBox();
                i=this.paper.text(s.x+i.actionsOffsetX,s.y+i.labelOffsetY+i.labelHeight+i.actionsOffsetY+n.coorYText,k);
                k=i.getBBox();
                i.attr(n.textNormal);
                i.translate(0,(k.height+(n.materiales.length>4?16:0))/2);
				n.coorYText+=k.height+(n.materiales.length>4?16:0);
				g.addInner(i);
        },
