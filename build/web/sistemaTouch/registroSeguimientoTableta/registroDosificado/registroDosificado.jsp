    ...
    function guardarDosificado()
    {
        ...
        if(!admin)
        {
            if(permisoEspDespiroge)
            {
                    for(var i=2;i<(especificaciones.rows.length);i++)
                    {
                        ...
                        if(especificaciones.rows[i].cells[3].getElementsByTagName('input')[0].type=='tel')
                        {
                            ...
                            if(especificaciones.rows[i].cells[3].getElementsByTagName('input')[0].id!='numEsp29')
                            {
            ...
            if(permisoPorosidad)
            {
                    ...
                    for(var i=1;i<tablePost.rows.length;i++)
                        {
                            if(parseInt(tablePost.rows[i].cells[4].getElementsByTagName('select')[0].value)>0)
                            {
                                ...
                                cont++;
                                dataPostF[cont]=encodeURIComponent(tablePost.rows[i].cells[2].getElementsByTagName('input')[0].value.split(',').join("$%"));
                                cont++;
                ...
                {
                         cont=0;
                         for(var i=2;i<tablaPre.rows.length;i++)
                            {
                                if(validarRegistroEntero(tablaPre.rows[i].cells[7].getElementsByTagName('input')[0]))
                                 {
                                        ...
                                        cont++;
                                        dataPreF[cont]=encodeURIComponent(tablaPre.rows[i].cells[5].getElementsByTagName('input')[0].value.split(",").join("|"));
                                        cont++;
                                        dataPreF[cont]=(tablaPre.rows[i].cells[7].getElementsByTagName('input')[0].value==''?"0":parseInt(tablaPre.rows[i].cells[7].getElementsByTagName('input')[0].value));
                                        cont++;
                                 }
                                 else
                                 {
            ...
            var peticion="ajaxGuardarSeguimientoDosificado.jsf?codLote="+codLote+"&noCache="+ Math.random()+
        ...
        arrayPersonal+="<option value='0' selected>-Seleccione una opción-</option>";
                ...
                try
                {
                    ...
                    String consulta="select pp.COD_TIPO_PROGRAMA_PROD,pp.COD_FORMULA_MAESTRA,pp.COD_COMPPROD,cp.COD_FORMA,p.nombre_prod,f.abreviatura_forma,cp.nombre_prod_semiterminado,cp.NOMBRE_GENERICO,f.nombre_forma,cp.REG_SANITARIO,"+
                                    ...
                                    " and afm2.COD_ACTIVIDAD = 259 and afm2.COD_FORMULA_MAESTRA =pp.COD_FORMULA_MAESTRA and afm2.COD_PRESENTACION=0" +
                    ...
                    if(res.next())
                    {
                        ...
                        if(codActividadReguladoEquipo==0||codActividadArmadoFiltros==0||codActividadCambioFormato==0)
                        {
                           out.println("<script>alert('No se encuentran asociadas una o mas de las siguientes actividades:Armado de Filtros, Bombas y envío de muestras, CAMBIO DE FORMATO ENVASADORA, REGULADO DEL EQUIPO PARA APROBACION DE VOLUMEN Y ALTURA');window.close();</script>");
                        }
...
<section class="main">
                         <div class="row"  style="margin-top:5px" >
                            ...
                            arrayPersonal="<option value='0'>--Ninguno--</option>"+UtilidadesTablet.operariosAreaProduccionAdminSelect(st, codAreaEmpresa, codPersonal, administrador);
                            
                            out.println("<script>operariosRegistro=\""+arrayPersonal+"\";fechaSistema=\""+sdfDias.format(new Date())+"\"</script>");
                            consulta="select t.COD_TAREA_OM from TAREAS_OM_PERSONAL_LOTE t where t.COD_LOTE='"+codLote+"' and t.COD_PROGRAMA_PROD='"+codprogramaProd+"' and t.COD_TAREA_OM in (7,8,9,10)" +
...
<div class="row"  style="margin-top:5px" >
            <div class="large-12 medium-12 small-12 large-centered medium-centered small-centered columns" >
                        ...
                        <div class="row">
                           <div class="large-12 medium-12 small-12 columns divHeaderClass" >
                                   <label  class="inline">DATOS PARA FILTRAR</label>
                            </div>
                        </div>
                        <div class="row" >
                        ...
                        <div class="row">
                           ...
                           <div class="row">
                              ...
                              <div class="large-1 medium-1 small-2 columns">
                                    <button <%=(administrador?"disabled":"")%> class="small button succes radius buttonAction" onclick="nuevoRegistro('registroCambioFormato')">+</button>
                              </div>
                              <div class="large-1 medium-1 small-2 columns">
                           ...
                           <table style="width:100%;margin-top:8px" id="registroArmadoFiltro" cellpadding="0px" cellspacing="0px">
                            ...
                            <div class="row">
                                <div class="large-5 medium-5 small-4 columns" style="">&nbsp;</div>
                              <div class="large-1 medium-1 small-2 columns">
                                    <button <%=(administrador?"disabled":"")%> class="small button succes radius buttonAction" onclick="nuevoRegistro('registroArmadoFiltro')">+</button>
                              </div>
                              <div class="large-1 medium-1 small-2 columns">
                    ...
                    <center>
                                ...
                                while(res.next())
                                {
                                    ...
                                    if(administrador)
                                    {
                                        ...
                                        out.println("<td class='tableCell' style='text-align:center;'>" +
                                                ...
                                                (res.getInt("RESULTADO_ESPERADO_LOTE")==1?"<input type='tel' id='numEsp"+res.getInt("COD_ESPECIFICACION_PROCESO")+"' "+(res.getInt("COD_ESPECIFICACION_PROCESO")==29?"disabled":"")+" value='"+(res.getInt("COD_ESPECIFICACION_PROCESO")==29?formatNum.format(cantidadAmpollasHora):res.getDouble("valorLote"))+"' style='width:5em !important'/>":"<input type='checkbox' id='conforme"+res.getRow()+"'  style='width:20px;height:20px' "+(res.getInt("conforme")>0?"checked":"")+"/>")+
                                                "</center></td>"+
                     ...
                     <table style="width:100%;margin-top:8px" id="reguladoEquipoVolumen" cellpadding="0px" cellspacing="0px">
                                    <tr >
                                       <td class="tableHeaderClass"  style="text-align:center" colspan="5">
                                           <span class="textHeaderClass">Regulado del equipo para aprobacion  de volumen y altura</span>
                                       </td>
                                     </tr>
                                   <tr >
                                       <td class="tableHeaderClass"  style="text-align:center">
                      ...
                      <div class="row">
                          <div class="large-5 medium-5 small-4 columns" style="">&nbsp;</div>
                          <div class="large-1 medium-1 small-2  columns">
                                <button <%=(administrador?"disabled":"")%> class="small button succes radius buttonAction" onclick="nuevoRegistro('reguladoEquipoVolumen')">+</button>
                          </div>
                           <div class="large-1 medium-1 small-2 columns">
