...
System.out.println("ccdc "+codSeguimiento);
...
try
{
     ...
     if(!admin)
     {
                ...
                if(pst.executeUpdate()>0)System.out.println("se eliminaron anteriores registros");
                 ...
                 double cantidadHorasHombre=0d;
                 for(int i=0;(i<dataControlDosificado.length&&dataControlDosificado.length>1);i+=9)
                 {
                     cont++;
                    cantidadAmpollas+=Double.valueOf(dataControlDosificado[i+1]);
                    cantidadHorasHombre+=Double.valueOf(dataControlDosificado[i+8]);
                    consulta="INSERT INTO SEGUIMIENTO_CONTROL_DOSIFICADO_LOTE_PERSONAL("+
                             ...
                             " CANT_AMPOLLAS_CC, CANT_AMPOLLAS_CARBONES, CANT_AMPOLLAS_ROTAS,COD_REGISTRO_ORDEN_MANUFACTURA)"+
                             " VALUES ('"+codSeguimiento+"','"+dataControlDosificado[i]+"','"+dataControlDosificado[i+1]+"',"+
                             " '"+dataControlDosificado[i+2]+"','"+dataControlDosificado[i+3]+"','"+dataControlDosificado[i+4]+"',"+
                             "'"+cont+"')";
             ...
             consulta="select isnull(max(s.COD_SEGUIMIENTO_DOSIFICADO_LOTE),0)as codSeguimientp from SEGUIMIENTO_DOSIFICADO_LOTE s where s.COD_PROGRAMA_PROD='"+codProgramaProd+"' and s.COD_LOTE='"+codLote+"'";
             res=st.executeQuery(consulta);
             int codSeguimientoDosificado=0;
             if(res.next())codSeguimientoDosificado=res.getInt("codSeguimientp");
             if(codSeguimientoDosificado>0)
             {
                 Double cantidadAmpollasHora=(cantidadHorasHombre>0?(cantidadAmpollas*100/cantidadHorasHombre):0);
                 cantidadAmpollasHora=(cantidadAmpollasHora.intValue()/100d);
                 consulta=" UPDATE SEGUIMIENTO_ESPECIFICACIONES_DOSIFICADO_LOTE" +
                          " SET VALOR_EXACTO='"+cantidadAmpollasHora+"'"+
                          " where  COD_SEGUIMIENTO_DOSIFICADO_LOTE='"+codSeguimientoDosificado+"'" +
                          " and COD_ESPECIFICACION_PROCESO =29 " +
                          " and COD_PERSONAL='"+codPersonalUsuario+"'";
                 System.out.println("consulta update seguimiento ampollas dosificadas "+consulta);
                 pst=con.prepareStatement(consulta);
                 if(pst.executeUpdate()>0)System.out.println("se actualizo la cantidad de ampollas por hora");
             }
     }
