...
//String cantidadCC=request.getParameter("cantidadCC");
con=Util.openConnection(con);
con.setAutoCommit(false);
String consulta="select max(s.COD_SEGUIMIENTO_RENDIMIENTO_LOTE_ACOND) as codSeguimiento"+
                " from SEGUIMIENTO_RENDIMIENTO_LOTE_ACOND s where s.COD_LOTE='"+codLote+"' and s.COD_PROGRAMA_PROD='"+codprogramaProd+"'" +
...
System.out.println("consulta buscar codSeguimiento "+consulta);
 ...
 if(codSeguimiento==0)
 {
    consulta="select isnull(max(s.COD_SEGUIMIENTO_RENDIMIENTO_LOTE_ACOND),0)+1 as codSeguimiento from SEGUIMIENTO_RENDIMIENTO_LOTE_ACOND s ";
    System.out.println("consulta codSeguimiento "+consulta);
    res=st.executeQuery(consulta);
    if(res.next())codSeguimiento=res.getInt("codSeguimiento");
    consulta="INSERT INTO SEGUIMIENTO_RENDIMIENTO_LOTE_ACOND(COD_LOTE,COD_COMPPROD,COD_TIPO_PROGRAMA_PROD,OBSERVACIONES,"+
             " COD_PERSONAL_SUPERVISOR,FECHA_CIERRE,COD_PROGRAMA_PROD,COD_SEGUIMIENTO_RENDIMIENTO_LOTE_ACOND,"+
             " COD_ESTADO_HOJA)VALUES ('"+codLote+"','"+codCompProd+"','"+codTipoProgramaProd+"','"+observacion+"','"+codPersonalUsuario+"',"+
             " '"+sdf.format(new Date())+"','"+codprogramaProd+"','"+codSeguimiento+"',0)";
     ...
     else
     {
             ...
             consulta="UPDATE SEGUIMIENTO_RENDIMIENTO_LOTE_ACOND"+
                      " SET OBSERVACIONES = '"+observacion+"',"+
                      " COD_PERSONAL_SUPERVISOR = '"+codPersonalUsuario+"',FECHA_CIERRE = '"+sdf.format(new Date())+"',"+
                      " COD_ESTADO_HOJA = 0" +
                      " WHERE COD_SEGUIMIENTO_RENDIMIENTO_LOTE_ACOND ='"+codSeguimiento+"'";
