...
<script type="text/javascript">
    ...
    function guardarRepesada()
    {
        ...
        for(var i=2;i<tabla.rows.length;i++)
         ...
         var peticion="ajaxGuardarSeguimientoRepesada.jsf?codLote="+codLote+"&noCache="+ Math.random()+
             ...
             "&fechaSeguimiento="+document.getElementById('fechaRepesada').innerHTML+
             "&horaInicio="+document.getElementById("horaRepesadaInicio").innerHTML+
             "&horaFinal="+document.getElementById("horaRepesadafinal").innerHTML+
             ...
             "&terminado="+(document.getElementById("buttonCerrado").className=='buttonFinishActive'?1:0)+
             "&admin="+(admin?"1":"0")+
    ...
    function calcularHorasGeneralRepesada(celdaFin,horaInicio)
    {
        celdaFin.className='buttonFinishActive';
        document.getElementById("horaRepesadafinal").innerHTML=getHoraActualGeneralString();
        ...
        try
        {
            document.getElementById("spanHorasHombre").innerHTML=redondeo2decimales(horasHombre);
        }
        catch(e)
        {
        ...
        out.println("<script>sqlConnection.verificarRegistroPendiente('"+codprogramaProd+"','"+codLote+"',2)</script>");
                try
                {
                    ...
                    String consulta="select pp.COD_COMPPROD,pp.COD_TIPO_PROGRAMA_PROD, isnull(afm.COD_ACTIVIDAD_FORMULA,0) as  COD_ACTIVIDAD_FORMULA, cp.COD_FORMA,p.nombre_prod,f.abreviatura_forma,cp.nombre_prod_semiterminado,cp.NOMBRE_GENERICO,f.nombre_forma,cp.REG_SANITARIO,"+
                                    ...
                                    " ,ISNULL(pp.COD_FORMULA_MAESTRA_VERSION,0) as codVersionFormula"+
                    ...
                    if(res.next())
                    {
                        codVersionFormula=res.getInt("codVersionFormula");
...
<div class="row"  style="margin-top:5px" >
            <div class="large-12 medium-12 small-12 large-centered medium-centered small-centered columns" >
                    ...
                    consulta="select P.COD_PERSONAL,(P.AP_PATERNO_PERSONAL + ' ' + P.AP_MATERNO_PERSONAL + ' ' +P.NOMBRES_PERSONAL + ' ' + P.nombre2_personal) NOMBRES_PERSONAL"+
                                ...
                                consulta="select fmdp.COD_FORMULA_MAESTRA_FRACCIONES,um.ABREVIATURA,"+
                                        " m.NOMBRE_MATERIAL,m.COD_MATERIAL,fmdp.CANTIDAD,ISNULL(srld.PESADA_CORRECTAMENTE,0) as PESADA_CORRECTAMENTE" +
                                        " ,f.CANTIDAD_LOTE"+
                                        " from FORMULA_MAESTRA_DETALLE_MP_VERSION fmd inner join FORMULA_MAESTRA_DETALLE_MP_FRACCIONES_VERSION fmdp"+
                                        " on fmd.COD_FORMULA_MAESTRA=fmdp.COD_FORMULA_MAESTRA and"+
                                        " fmd.COD_MATERIAL=fmdp.COD_MATERIAL and fmd.COD_VERSION=fmdp.COD_VERSION" +
                                        ...
                                        " where fmd.COD_FORMULA_MAESTRA='"+codFormulaMaestra+"' and fmd.COD_VERSION='"+codVersionFormula+"'"+
                                ...
                                while(res.next())
                                {
                                    %>
                                     <tr >
                                                   ...
                                                   <td class="tableCell" style="text-align:center;">
                                                       <span class="textHeaderClassBody" style="font-weight:normal"><%=format.format((cantidadAmpollas/res.getDouble("CANTIDAD_LOTE"))*res.getDouble("CANTIDAD"))+" "+res.getString("ABREVIATURA")%></span>
                                                   </td>
                                                   <td class="tableCell"  style="text-align:center;">
                    ...
                    <table style="width:80%;margin-top:2px;border-bottom:solid #a80077 1px;" id="datosAdicionales" cellpadding="0px" cellspacing="0px" >
                        ...
                        <%
                            ...
                            consulta="select sppp.FECHA_INICIO,sppp.FECHA_FINAL,sppp.HORAS_HOMBRE,sppp.COD_PERSONAL,sppp.REGISTRO_CERRADO "+
                            ...
                            if(res.next())
                            {
                                ...
                                registroCerrado=(res.getInt("REGISTRO_CERRADO")>0);
                            }
                         %>
                         <tr >
                        ...
                        <tr>
                             <td style="border-left:solid #a80077 1px;text-align:left">
                                   <span >FECHA REPESADA</span>
                                    ...
                                    <td style="border-right:solid #a80077 1px;text-align:left">
                                        <span id="fechaRepesada" class='textHeaderClassBodyNormal' ><%=(sdfDias.format(fechaInicio))%></span>
                          ...
                          <tr>
                             <td style="border-left:solid #a80077 1px;text-align:left">
                                   <span >Hora Inicio</span>
                                    ...
                                    <td style="border-right:solid #a80077 1px;text-align:left">
                                        <span id="horaRepesadaInicio" class='textHeaderClassBodyNormal' ><%=(sdfHoras.format(fechaInicio))%></span>
                                    </td>
                         </tr>
                         <tr>
                             <td style="border-left:solid #a80077 1px;text-align:left">
                                   <span >Hora Final</span>
                                    ...
                                    <td style="border-right:solid #a80077 1px;text-align:left">
                                        <button  id="buttonCerrado" class="<%=(registroCerrado?"buttonFinishActive":"buttonFinish")%>"
                                        onclick="calcularHorasGeneralRepesada(this,'<%=(sdfHoras.format(fechaInicio))%>')" >Terminar</button>
                                        <span class='textHeaderClassBodyNormal' id="horaRepesadafinal"><%=(sdfHoras.format(fechaFinal))%></span>
                                    </td>
                            </tr>
                            <tr>
                             <td style="border-left:solid #a80077 1px;text-align:left">
                    ...
                    <input type="hidden" value="<%=(registroCerrado?codPersonalSupervisor:0)%>" id="cerrado">
    ...
    <div  id="formsuper"  style="
        ...
        <script src="../../reponse/js/registroDesviacion.js"></script>
        <script>loginHoja.verificarHojaCerrada('cerrado', admin,'codProgramaProd','codLoteSeguimiento',2,<%=(codEstadoHoja)%>);</script>
