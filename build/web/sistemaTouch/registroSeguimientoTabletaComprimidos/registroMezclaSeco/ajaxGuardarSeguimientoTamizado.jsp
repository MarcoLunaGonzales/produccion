package sistemaTouch.registroSeguimientoTabletaComprimidos.registroTamizado_1;
...
String[] dataTamizado=request.getParameter("dataTamizado").split(",");
String[] dataMezclado=request.getParameter("dataMezclado").split(",");
...
String codActividadMezclado=request.getParameter("codActividadMezclado");
...
try
{
     ...
     consulta="SELECT MAX(S.COD_SEGUIMIENTO_TAMIZADO_LOTE) as codSeguimiento FROM SEGUIMIENTO_TAMIZADO_LOTE S WHERE S.COD_PROGRAMA_PROD='"+codProgProd+"' AND S.COD_LOTE='"+codLote+"'";
     Statement st=con.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);
     ResultSet res=st.executeQuery(consulta);
     if(res.next())codSeguimiento=res.getInt("codSeguimiento");
     if(codSeguimiento==0)
     {
         consulta="select isnull(max(s.COD_SEGUIMIENTO_TAMIZADO_LOTE),0)+1 as codSeg from  SEGUIMIENTO_TAMIZADO_LOTE s";
          res=st.executeQuery(consulta);
          if(res.next())codSeguimiento=res.getInt("codSeg");
          consulta="INSERT INTO SEGUIMIENTO_TAMIZADO_LOTE(COD_SEGUIMIENTO_TAMIZADO_LOTE, COD_LOTE,COD_PROGRAMA_PROD,COD_ESTADO_HOJA"+
     ...
     else
     {
         if(administrador)
         {
             consulta="UPDATE SEGUIMIENTO_TAMIZADO_LOTE SET COD_PERSONAL_SUPERVISOR =?,"+
                      " FECHA_CIERRE = '"+sdf.format(new Date())+"',OBSERVACION = ?,COD_ESTADO_HOJA = 0"+
                      " WHERE COD_SEGUIMIENTO_TAMIZADO_LOTE =?";
     ...
     if(!administrador)
    {
             consulta="delete SEGUIMIENTO_ESPECIFICACIONES_TAMIZADO_LOTE  where COD_SEGUIMIENTO_TAMIZADO_LOTE='"+codSeguimiento+"' and COD_PERSONAL='"+codPersonalUsuario+"'";
             ...
             {
                 consulta="INSERT INTO SEGUIMIENTO_ESPECIFICACIONES_TAMIZADO_LOTE("+
                         " COD_SEGUIMIENTO_TAMIZADO_LOTE, COD_PERSONAL, COD_ESPECIFICACION_PROCESO,"+
             ...
             consulta="delete SEGUIMIENTO_PROGRAMA_PRODUCCION where COD_PROGRAMA_PROD='"+codProgProd+"'"+
                            ...
                            " COD_ACTIVIDAD_PROGRAMA in ('"+codActividadTamizado+"','"+codActividadMezclado+"')";
            ...
            consulta="delete SEGUIMIENTO_PROGRAMA_PRODUCCION_PERSONAL where COD_PROGRAMA_PROD='"+codProgProd+"'"+
                    " and COD_LOTE_PRODUCCION='"+codLote+"' and COD_COMPPROD='"+codCompProd+"'"+
                    " and COD_FORMULA_MAESTRA='"+codFormula+"' and COD_TIPO_PROGRAMA_PROD='"+codTipoProgramaProd+"' and " +
                    " COD_PERSONAL='"+codPersonalUsuario+"' and "+
                    " COD_ACTIVIDAD_PROGRAMA in ('"+codActividadTamizado+"','"+codActividadMezclado+"')";
            System.out.println("consulta delete seguimiento personal "+consulta);
            pst=con.prepareStatement(consulta);
            if(pst.executeUpdate()>0)System.out.println("se eliminaron anteriores registros");
            for(int i=0;(i<dataTamizado.length&&dataTamizado.length>1);i+=5)
             {
                 String[] aux=dataTamizado[i].split("/");
                String fechaInicio=aux[2]+"/"+aux[1]+"/"+aux[0]+" "+dataTamizado[i+1];
                String fechaFinal=aux[2]+"/"+aux[1]+"/"+aux[0]+" "+dataTamizado[i+2];
                ...
                consulta = " INSERT INTO SEGUIMIENTO_PROGRAMA_PRODUCCION_PERSONAL( COD_COMPPROD,  COD_PROGRAMA_PROD,  COD_LOTE_PRODUCCION,  COD_FORMULA_MAESTRA," +
                            ...
                            "'"+codProgProd+"','"+codLote+"'," +
                            "'"+codFormula+"','"+codActividadTamizado+"'," +
                            " '"+codTipoProgramaProd+"'," +
                            " '"+codPersonalUsuario+"','"+dataTamizado[i+3]+"','0'" +
                            ",'"+fechaInicio+":00','"+fechaInicio+":00','"+fechaFinal+":00'" +
                            ",'0','0','0','"+dataTamizado[i+4]+"')";
            ...
            for(int i=0;(i<dataMezclado.length&&dataMezclado.length>1);i+=5)
             {
                 String[] aux=dataMezclado[i].split("/");
                String fechaInicio=aux[2]+"/"+aux[1]+"/"+aux[0]+" "+dataMezclado[i+1];
                String fechaFinal=aux[2]+"/"+aux[1]+"/"+aux[0]+" "+dataMezclado[i+2];

                consulta = " INSERT INTO SEGUIMIENTO_PROGRAMA_PRODUCCION_PERSONAL( COD_COMPPROD,  COD_PROGRAMA_PROD,  COD_LOTE_PRODUCCION,  COD_FORMULA_MAESTRA," +
                            "  COD_ACTIVIDAD_PROGRAMA,  COD_TIPO_PROGRAMA_PROD,  COD_PERSONAL,  HORAS_HOMBRE,  UNIDADES_PRODUCIDAS,  FECHA_REGISTRO,  FECHA_INICIO," +
                            "  FECHA_FINAL,HORAS_EXTRA,UNIDADES_PRODUCIDAS_EXTRA,COD_REGISTRO_ORDEN_MANUFACTURA,REGISTRO_CERRADO) VALUES ( '"+codCompProd+"'," +
                            "'"+codProgProd+"','"+codLote+"'," +
                            "'"+codFormula+"','"+codActividadMezclado+"'," +
                            " '"+codTipoProgramaProd+"'," +
                            " '"+codPersonalUsuario+"','"+dataMezclado[i+3]+"','0'" +
                            ",'"+fechaInicio+":00','"+fechaInicio+":00','"+fechaFinal+":00'" +
                            ",'0','0','0','"+dataMezclado[i+4]+"')";
                System.out.println("consulta insert seguimiento programa produccion personal "+consulta);
                pst=con.prepareStatement(consulta);
                if(pst.executeUpdate()>0)System.out.println("se registro el seguimiento Personal");
             }
            consulta="INSERT INTO SEGUIMIENTO_PROGRAMA_PRODUCCION( COD_COMPPROD,  COD_PROGRAMA_PROD,COD_LOTE_PRODUCCION,COD_FORMULA_MAESTRA,"+
                              ...
                              " and spp.COD_ACTIVIDAD_PROGRAMA in ('"+codActividadTamizado+"','"+codActividadMezclado+"')"+
