...
ResultSet resDetalle;
    ...
    consulta=" select maf.COD_MAQUINA,max(maf.HORAS_MAQUINA) as MaxHoras,af.ORDEN_ACTIVIDAD,"+
             " (select ISNULL(MAX(ppcd.FECHA_FINAL), '"+currentFecha+"') as fechaInicio"+
             " from PROGRAMA_PRODUCCION_CRONOGRAMA_DETALLE ppcd"+
             " INNER JOIN PROGRAMA_PRODUCCION_CRONOGRAMA ppc on"+
             " ppc.COD_PROGRAMA_PRODUCCION_CRONOGRAMA =ppcd.COD_PROGRAMA_PRODUCCION_CRONOGRAMA"+
             " where ppcd.COD_MAQUINA =maf.COD_MAQUINA and"+
             " ppcd.FECHA_FINAL > '"+currentFecha+"') as fechaMax"+
             " from ACTIVIDADES_PRODUCCION ap inner join ACTIVIDADES_FORMULA_MAESTRA af on"+
             " ap.COD_ACTIVIDAD=af.COD_ACTIVIDAD inner join FORMULA_MAESTRA fm"+
             " on fm.COD_FORMULA_MAESTRA=af.COD_FORMULA_MAESTRA"+
             " and af.COD_ESTADO_REGISTRO=1 inner join MAQUINARIA_ACTIVIDADES_FORMULA maf"+
             " on maf.COD_ACTIVIDAD_FORMULA=af.COD_ACTIVIDAD_FORMULA and maf.COD_ESTADO_REGISTRO=1"+
             " where af.cod_formula_maestra = '"+cod[2]+"' and af.COD_AREA_EMPRESA = 40 and maf.COD_MAQUINA<>98 " +
             " and ap.COD_ACTIVIDAD in (133,134,136,137,139,166,174,180,250) " +
             " group by maf.COD_MAQUINA,af.ORDEN_ACTIVIDAD order by af.ORDEN_ACTIVIDAD";
    ...
    if(res.next())
            {
                ...
                horasMaq=res.getDouble("MaxHoras");
                fechafinal=sumarFechaHoras(fechaInicio,horasMaq);
                consulta="INSERT INTO PROGRAMA_PRODUCCION_CRONOGRAMA_DETALLE(COD_PROGRAMA_PRODUCCION_CRONOGRAMA, COD_MAQUINA, FECHA_INICIO, FECHA_FINAL)"+
                         " VALUES ('"+codProgProdCron+"','"+res.getString("COD_MAQUINA")+"','"+sdf.format(fechaInicio)+"','"+sdf.format(fechafinal)+"')";
    ...
    consulta=" select maf.COD_MAQUINA,max(maf.HORAS_MAQUINA) as MaxHoras,af.ORDEN_ACTIVIDAD,"+
             ...
             " and af.COD_ESTADO_REGISTRO=1 inner join MAQUINARIA_ACTIVIDADES_FORMULA maf"+
             " on maf.COD_ACTIVIDAD_FORMULA=af.COD_ACTIVIDAD_FORMULA and maf.COD_ESTADO_REGISTRO=1"+
             " where af.cod_formula_maestra = '"+cod[2]+"' and af.COD_AREA_EMPRESA = 96 and maf.COD_MAQUINA<>98" +
             " group by maf.COD_MAQUINA,af.ORDEN_ACTIVIDAD order by af.ORDEN_ACTIVIDAD";
    System.out.println("consulta cargar maquinarias de la formula "+consulta);
    res=st.executeQuery(consulta);
