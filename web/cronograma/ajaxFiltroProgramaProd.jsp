...
String consulta = " select (CAST(pp.COD_PROGRAMA_PROD as varchar)+' '+CAST(pp.COD_COMPPROD as varchar)+' '+CAST(pp.COD_FORMULA_MAESTRA as varchar)"+
                  " +' '+CAST(pp.COD_TIPO_PROGRAMA_PROD AS VARCHAR)) AS CODIGOS,pp.COD_LOTE_PRODUCCION,cp.nombre_prod_semiterminado,tpp.NOMBRE_TIPO_PROGRAMA_PROD,isnull(tcf.nombre_tipo_formato_maquina,0) nombre_tipo_formato_maquina" +
                  ",tp.nombre_clasificacion_producto "+
                  ...
                  " left outer join tipos_formato_maquinaria tcf on tcf.cod_tipo_formato_maquina = cp.cod_tipo_compprod_formato" +
                  " left outer join tipos_clasificacion_producto tp on tp.cod_clasificacion_producto = cp.cod_tipo_clasificacion_producto "+
                  ...
                  " from PROGRAMA_PRODUCCION_CRONOGRAMA ppc where ppc.COD_ESTADO_PROGRAMA_PRODUCCION_CRONOGRAMA in (1,3)) ORDER BY tp.cod_clasificacion_producto desc,cod_tipo_formato_maquina desc";
...
out.println(" <tr class='headerClassACliente'><td></td> <td height='35px'>"+
            "PRODUCTO</td><td>LOTE</td><td>TIPO PROGRAMA PRODUCCION</td>" +
            "<td>TIPO FORMATO TABLETEADO</td><td>CLASIFICACION</td></tr>");
while(rs.next())
{
    ...
    out.println("<td>"+rs.getString("NOMBRE_TIPO_FORMATO_MAQUINA")+"</td>");
    out.println("<td>"+(rs.getString("NOMBRE_CLASIFICACION_PRODUCTO")==null?"":rs.getString("NOMBRE_CLASIFICACION_PRODUCTO"))+"</td>");
    out.println("</tr>");
}
out.println("</table>");
rs.close();
