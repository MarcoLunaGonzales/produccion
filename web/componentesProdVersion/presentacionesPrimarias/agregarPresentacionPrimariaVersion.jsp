    ...
    <a4j:form id="form1"  >
        <div style="text-align:center">
            ...
            <rich:panel headerClass="headerClassACliente" style="width:50%;margin-top:1em">
                <f:facet name="header">
                    <h:outputText value="Agregar Presentación Primaria"/>
                </f:facet>
                <h:panelGrid columns="3" >

                    <h:outputText value="Envase Primario" styleClass="outputTextBold"/>
                    <h:outputText value="::" styleClass="outputTextBold"/>
                    <h:selectOneMenu styleClass="inputText" value="#{ManagedComponentesProdVersion.presentacionesPrimariaRegistrar.envasesPrimarios.codEnvasePrim}">
                    ...
                    <h:outputText value="::" styleClass="outputTextBold"/>
                    <h:inputText value="#{ManagedComponentesProdVersion.presentacionesPrimariaRegistrar.cantidad}" styleClass="inputText"
                    onkeypress="valNum();" id="cantidad"/>
                    <h:outputText value="Tipo Programa Producción" styleClass="outputTextBold"/>
                    <h:outputText value="::" styleClass="outputTextBold"/>
                    <h:selectOneMenu value="#{ManagedComponentesProdVersion.presentacionesPrimariaRegistrar.tiposProgramaProduccion.codTipoProgramaProd}" styleClass="inputText">
            ...
            <center style="margin-top:1em;">
                <a4j:commandButton value="Guardar" action="#{ManagedComponentesProdVersion.guardarNuevaPresentacionPrimaria_action}" styleClass="btn"
                onclick="if(!validarGuardar()){return false;}"
                oncomplete="if(#{ManagedComponentesProdVersion.mensaje eq '1'}){alert('Se guardo la presentacion primaria');window.location.href='navegadorPresentacionesPrimariasVersion.jsf?reg='+(new Date()).getTime().toString();}else{alert('#{ManagedComponentesProdVersion.mensaje}');}"/>
