...
public void datosFormulaMaestra(String nombreMaterialFM,double cantidadFM,String abreviaturaFM,int codFormulaMaestra,int codMaterial,int codCompProd,int codPresentacion){
    
    try{
    String consulta =   " select top 1 m2.NOMBRE_MATERIAL, fmdes.CANTIDAD, um2.ABREVIATURA, prp.cod_presentacion, prp.NOMBRE_PRODUCTO_PRESENTACION" +
                        " from formula_maestra fm" +
                        " inner join FORMULA_MAESTRA_DETALLE_ES fmdes on fmdes.COD_FORMULA_MAESTRA = '"+codFormulaMaestra+"'" +
                        " inner join materiales m2 on m2.COD_MATERIAL = '"+codMaterial+"'" +
                        ...
                        " where fm.COD_COMPPROD = '"+codCompProd+"' and prp.cod_presentacion = '"+codPresentacion+"' and m2.COD_MATERIAL = '"+codMaterial+"' ";
    System.out.println("consulta "+ consulta);
    Connection con = null;
    con = Util.openConnection(con);
    Statement  st = con.createStatement();
    ResultSet rs = st.executeQuery(consulta);
    if(rs.next()){
        nombreMaterialFM = rs.getString("nombre_material");
        cantidadFM= rs.getDouble("cantidad");
        abreviaturaFM = rs.getString("abreviatura");
        //datos[3]=rs.getString("cod_presentacion");
        //datos[4] = rs.getString("nombre_producto_presentacion");
    }
    rs.close();
    st.close();
    con.close();
        ...
        while (i.hasNext()){
            ...
            String sql  = " SELECT top 1 (select c.nombre_prod_semiterminado from COMPONENTES_PROD c where c.COD_COMPPROD = s.COD_PROD),s.COD_PROD";                    
                   ...
                   sql = " select distinct s.COD_PROD,cp.nombre_prod_semiterminado,fm.cod_formula_maestra," +
                         ...
                         " where iad.COD_LOTE_PRODUCCION = s.cod_lote_produccion and iad.COD_COMPPROD = s.cod_prod and ia.COD_TIPOINGRESOACOND = 1 and ia.COD_ALMACENACOND in (1,3) and ia.cod_estado_ingresoacond not in(1,2)) cant_ingreso_produccion,fm.cantidad_lote,tppr.nombre_tipo_programa_prod " +
                         ", ppr.cod_tipo_programa_prod cod_tipo_programa_prod,0 cantidadEnviadaAPT from SALIDAS_ALMACEN s inner join COMPONENTES_PROD cp on cp.COD_COMPPROD = s.COD_PROD" +
                         ...
                         " inner join tipos_programa_produccion tppr on tppr.cod_tipo_programa_prod = ppr.cod_tipo_programa_prod where s.COD_LOTE_PRODUCCION = '"+bean.getCodLoteProduccion()+"' ";
            ...
            while (rs.next()){
                ...
                if(estaValor(materials1,"1")==1){
                        ...
                        consulta = "select nombre_material,abreviatura,sum(salidas_material) salidas_material,sum(devoluciones_material) devoluciones_material,nombre_material_fm,sum(cantidad_fm) cantidad_fm,abreviatura_fm,sum(cantidad_fm1) cantidad_fm1 from (" +
                                ...
                                " from devoluciones d" +
                                    ...
                                    " and d.cod_almacen = '1' " +
                                    " and ia.cod_estado_ingreso_almacen = 1 and d.cod_estado_devolucion = 1" +
                                    " and m1.cod_material = m.cod_material and d.COD_SALIDA_ALMACEN = sa1.COD_SALIDA_ALMACEN),0) devoluciones_material,m.nombre_material nombre_material_fm,0 cantidad_fm,um.abreviatura abreviatura_fm,0 cantidad_fm1" +
                                " from SALIDAS_ALMACEN sa1" +
                                " inner join SALIDAS_ALMACEN_DETALLE sad on sad.COD_SALIDA_ALMACEN = sa1.COD_SALIDA_ALMACEN" +
                                " inner join materiales m on m.COD_MATERIAL = sad.COD_MATERIAL" +
                                " inner join unidades_medida um on um.cod_unidad_medida = m.cod_unidad_medida" +
                                ...
                                " inner join materiales m on m.COD_MATERIAL = fmdmp.COD_MATERIAL" +
                                " inner join UNIDADES_MEDIDA um on um.COD_UNIDAD_MEDIDA = fmdmp.COD_UNIDAD_MEDIDA" +
                                " where fm.COD_COMPPROD = '"+codCompProd+"' and fm.cod_formula_maestra = '"+codFormulaMaestra+"' and fm.cod_estado_registro = 1" +
                                " ) as tabla group by nombre_material,abreviatura,nombre_material_fm,abreviatura_fm ";
                    ...
                    if(this.estaValor(materials1, "2")==1){
                            ...
                            consulta = "select nombre_material,abreviatura,sum(salidas_material) salidas_material,sum(devoluciones_material) devoluciones_material,nombre_material_fm,sum(cantidad_fm) cantidad_fm,abreviatura,sum(cantidad_fm1) cantidad_fm1 from (" +
                                    ...
                                    " isnull((select sum(isnull(iad.CANT_TOTAL_INGRESO_FISICO, 0)) from devoluciones d" +
                                        ...
                                        " and d.cod_almacen = '1' " +
                                        " and ia.cod_estado_ingreso_almacen = 1 and d.cod_estado_devolucion = 1" +
                                        " and m1.cod_material = m.cod_material and d.COD_SALIDA_ALMACEN = sa1.COD_SALIDA_ALMACEN),0) devoluciones_material,m.nombre_material nombre_material_fm,0 cantidad_fm,um.abreviatura abreviatura_fm,0 cantidad_fm1" + //fmx.nombre_material nombre_material_fm,fmx.cantidad cantidad_fm,fmx.abreviatura abreviatura_fm
                                    " from SALIDAS_ALMACEN sa1" +
                                    " inner join SALIDAS_ALMACEN_DETALLE sad on sad.COD_SALIDA_ALMACEN = sa1.COD_SALIDA_ALMACEN" +
                                    " inner join materiales m on m.COD_MATERIAL = sad.COD_MATERIAL" +
                                    " inner join unidades_medida um on um.cod_unidad_medida = m.cod_unidad_medida " +
                                    ...
                                    " where g.COD_GRUPO = m1.COD_GRUPO and g.COD_CAPITULO = 3) and sa1.COD_ESTADO_SALIDA_ALMACEN = 1 and sa1.ESTADO_SISTEMA = 1 " + //and ppr.cod_tipo_programa_prod = '"+codTipoProgramaProd+"'
                                    " union all " +
                                    " select m.NOMBRE_MATERIAL,um.ABREVIATURA,0,0,m.NOMBRE_MATERIAL,fmdmp.cantidad,um.ABREVIATURA,fmdmp.cantidad cantidad_fm1" +
                                    " from programa_produccion ppr inner join COMPONENTES_PROD cp on cp.cod_compprod = ppr.cod_compprod and ppr.cod_lote_produccion = '"+bean.getCodLoteProduccion()+"'" +
                                    " inner join FORMULA_MAESTRA fm on fm.COD_COMPPROD = cp.COD_COMPPROD" +
                                    " inner join PRESENTACIONES_PRIMARIAS prp on prp.COD_COMPPROD = ppr.COD_COMPPROD and prp.COD_TIPO_PROGRAMA_PROD = ppr.COD_TIPO_PROGRAMA_PROD " +
                                    " inner join FORMULA_MAESTRA_DETALLE_EP fmdmp on fmdmp.COD_FORMULA_MAESTRA = fm.COD_FORMULA_MAESTRA and fmdmp.COD_PRESENTACION_PRIMARIA = prp.COD_PRESENTACION_PRIMARIA " +
                ...
                if(this.estaValor(materials1, "3")==1){
                                ...
                                consulta = "select nombre_material,abreviatura,sum(salidas_material) salidas_material,sum(devoluciones_material) devoluciones_material,nombre_material_fm,sum(cantidad_fm) cantidad_fm,abreviatura_fm,cod_presentacion,sum(cantidad_fm1) cantidad_fm1,NOMBRE_PRODUCTO_PRESENTACION from (" +
                                        ...
                                        " and d.cod_almacen = '2' " +
                                        " and ia.cod_estado_ingreso_almacen = 1 and d.cod_estado_devolucion = 1" +
                                        " and m1.cod_material = m.cod_material and d.COD_SALIDA_ALMACEN = sa1.COD_SALIDA_ALMACEN),0) devoluciones_material,m.nombre_material nombre_material_fm,0 cantidad_fm,um.abreviatura abreviatura_fm,prp.cod_presentacion,0 cantidad_fm1,prp.NOMBRE_PRODUCTO_PRESENTACION" + //fmx.nombre_material nombre_material_fm,fmx.cantidad cantidad_fm,fmx.abreviatura abreviatura_fm
                                        " from SALIDAS_ALMACEN sa1" +
                                        " inner join SALIDAS_ALMACEN_DETALLE sad on sad.COD_SALIDA_ALMACEN = sa1.COD_SALIDA_ALMACEN" +
                                        " inner join materiales m on m.COD_MATERIAL = sad.COD_MATERIAL" +
                                        " inner join unidades_medida um on um.cod_unidad_medida = m.cod_unidad_medida" +
                                        " inner join presentaciones_producto prp on prp.cod_presentacion = sa1.cod_presentacion" +
                                        " inner join componentes_presprod cprp on cprp.cod_presentacion = prp.cod_presentacion and cprp.cod_compprod = '"+codCompProd+"' and cprp.cod_tipo_programa_prod = '"+codTipoProgramaProd+"' " +
                                        ...
                                        " where g.COD_GRUPO = m1.COD_GRUPO and g.COD_CAPITULO in(4,8)) and sa1.COD_ESTADO_SALIDA_ALMACEN = 1 and sa1.ESTADO_SISTEMA = 1 " + //and ppr.cod_tipo_programa_prod = '"+codTipoProgramaProd+"'
                                        " union all " +
                                        " select m.NOMBRE_MATERIAL,um.ABREVIATURA,0,0,m.NOMBRE_MATERIAL,fmdmp.cantidad cantidad_fm,um.ABREVIATURA,prp.cod_presentacion,fmdmp.CANTIDAD cantidad_fm1,prp.nombre_producto_presentacion" +//fmx.nombre_material nombre_material_fm,fmx.cantidad cantidad_fm,fmx.abreviatura abreviatura_fm
                                        " from programa_produccion ppr inner join COMPONENTES_PROD cp on cp.cod_compprod = ppr.cod_compprod and ppr.cod_lote_produccion = '"+bean.getCodLoteProduccion()+"'" +
                                        " inner join COMPONENTES_PRESPROD cprp on cprp.COD_COMPPROD = ppr.COD_COMPPROD and cprp.COD_TIPO_PROGRAMA_PROD = ppr.COD_TIPO_PROGRAMA_PROD " +
                                        //" inner join salidas_almacen s on s.cod_lote_produccion = ppr.cod_lote_produccion and s.cod_prod = ppr.cod_compprod and s.cod_almacen = 2"+
                                        " inner join FORMULA_MAESTRA fm on fm.COD_COMPPROD = cp.COD_COMPPROD" +
                                        ...
                                        " where fm.COD_COMPPROD = '"+codCompProd+"' and fm.cod_estado_registro = 1  and prp.cod_tipo_programa_prod = '"+codTipoProgramaProd+"' " + //and prp.cod_tipo_programa_prod = '"+codTipoProgramaProd+"' //and ppr.cod_tipo_programa_prod = '"+codTipoProgramaProd+"'
                                        ") as tabla group by nombre_material,abreviatura,nombre_material_fm,abreviatura_fm,cod_presentacion,nombre_producto_presentacion" +
                                        " order by nombre_producto_presentacion";
                                /*
                                    (select sum(sd.CANTIDAD_SALIDA_ALMACEN) salidas" +
                                ...
                                */
                                ...
                                /*select top 1 m2.NOMBRE_MATERIAL, fmdmp.CANTIDAD, um2.ABREVIATURA, prp.cod_presentacion, prp.NOMBRE_PRODUCTO_PRESENTACION" +
                                        " from programa_produccion ppr" +
                                        " left outer join formula_maestra fm on fm.COD_FORMULA_MAESTRA = ppr.COD_FORMULA_MAESTRA" +
                                        " left outer join formula_maestra_detalle_es fmdmp on fmdmp.cod_formula_maestra = fm.cod_formula_maestra and fmdmp.cod_material = m.COD_MATERIAL and" +
                                        " fmdmp.COD_PRESENTACION_PRODUCTO = sa1.COD_PRESENTACION" +
                                        ...
                                        " left outer join PRESENTACIONES_PRODUCTO prp on prp.cod_presentacion = fmdmp.COD_PRESENTACION_PRODUCTO" +
                                        " where fm.cod_compprod = sa1.COD_PROD and ppr.cod_compprod =  sa1.COD_PROD and ppr.cod_lote_produccion = sa1.COD_LOTE_PRODUCCION and ppr.cod_estado_programa in (1, 2,6, 7)" +
                                        " order by fm.COD_FORMULA_MAESTRA desc*/
