...
<%!    public double compara(int unidad1, int unidad2) {
        ...
        try {
            ...
            Statement st = con7.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
            ...
            if (sw == 1) {
                ...
                Statement st2 = con7.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
...
public String proveedorUltimaCompra(String codMaterial){
    ...
    Statement st = con6.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
    ResultSet rs = st.executeQuery(consulta);
    if(rs.next()){
...
public Double loteMaterial(String codMaterial){
    ...
    System.out.println("consulta " + consulta);
    Statement st = con4.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
    ResultSet rs = st.executeQuery(consulta);
    if(rs.next()){
...
public Double cantidadLoteEP(String codMaterial){
    Double cantidadLoteMaterial = 0.0;
    Connection con5=null;
    try{
    con5 = Util.openConnection(con5);
    String consulta= "select max(fmd.CANTIDAD) as CANTIDAD"+
                     " from FORMULA_MAESTRA fm inner join FORMULA_MAESTRA_DETALLE_EP fmd on "+
                     " fm.COD_FORMULA_MAESTRA=fmd.COD_FORMULA_MAESTRA inner join PRESENTACIONES_PRIMARIAS pp "+
                     " on pp.COD_COMPPROD=fm.COD_COMPPROD" +
                     " where fmd.cod_material='"+codMaterial+"'";
    System.out.println("consulta " + consulta);
    con5 = Util.openConnection(con5);
    Statement sta = con5.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
    ResultSet res = sta.executeQuery(consulta);
    if(res.next()){
        cantidadLoteMaterial= res.getDouble("cantidad");
    }con5.close();
    }catch(Exception e){
        e.printStackTrace();
    }
    return cantidadLoteMaterial;
}
%>
<html>
    <head>
    ...
    <body>
        ...
        System.out.println("entro reportes");
        ...
        try {
            ...
            System.out.println("termino iteracion");
            items += " 0 ";
            ...
            sql_exp += " (select SUM(iade.cantidad_restante)" +
                    ...
                    " and iade.cod_material = iad.cod_material and iade.cod_ingreso_almacen = iad.cod_ingreso_almacen " +
                    " and ia.fecha_ingreso_almacen <= '" +fecha_existencia+" "+hora+"' " +
                    " and iade.cod_estado_material in (1,2, 6,8) and iade.cantidad_restante > 0 and ia.cod_almacen in(1,2,9))as aprobados,";
                ...
                while (rs2.next()) {
                    ...
                    if(codCapitulo==4 || codCapitulo == 8){
                        String consulta= " select  fd.CANTIDAD from FORMULA_MAESTRA f, FORMULA_MAESTRA_DETALLE_ES fd, " +
                                        ...
                                        " f.COD_FORMULA_MAESTRA=fd.COD_FORMULA_MAESTRA and fd.COD_MATERIAL=" + codMaterial + " and cp.COD_PRESENTACION=p.cod_presentacion " +
                                        " and cp.COD_COMPPROD=f.COD_COMPPROD and p.COD_CATEGORIA=c.COD_CATEGORIA order by p.COD_CATEGORIA  ";
                       System.out.println("consulta tam lote " + consulta);
                        Statement st = con.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
                        ResultSet rs = st.executeQuery(consulta);
                        if(rs.next()){
                            tamLote = rs.getDouble("cantidad");
                        }
                    ...
                    if(codCapitulo==3)
                    {
                        ...
                        ResultSet res = sta.executeQuery(consulta);
                        if(res.next()){
                            tamLote= res.getDouble("cantidad");
                        }
                    }
