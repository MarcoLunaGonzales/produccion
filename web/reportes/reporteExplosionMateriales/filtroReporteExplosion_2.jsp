...
public double loteItem(String codMaterial){
    double cantidadLote = 0.0;
    try{
        Connection con = null;
        con = Util.openConnection(con);
        Statement st = con.createStatement();
        ResultSet rs = st.executeQuery(" select top 1 f.CANTIDAD cantidad_lote from FORMULA_MAESTRA_DETALLE_EP f where f.COD_MATERIAL = 555" +
                                       " order by f.CANTIDAD desc");
        if(rs.next()){
            cantidadLote = rs.getDouble("cantidad_lote");
        }
    }catch(Exception e){e.printStackTrace();}
      return cantidadLote;
}
public Double loteMaterial(String codMaterial){
    Double loteMaterial = 0.0;
    try{
    String consulta= " select  fd.CANTIDAD from FORMULA_MAESTRA f, FORMULA_MAESTRA_DETALLE_ES fd, " +
                            " COMPONENTES_PRESPROD cp, PRESENTACIONES_PRODUCTO p, CATEGORIAS_PRODUCTO c where " +
                            " f.COD_FORMULA_MAESTRA=fd.COD_FORMULA_MAESTRA and fd.COD_MATERIAL=" + codMaterial + " and cp.COD_PRESENTACION=p.cod_presentacion " +
                            " and cp.COD_COMPPROD=f.COD_COMPPROD and p.COD_CATEGORIA=c.COD_CATEGORIA order by p.COD_CATEGORIA  ";
    System.out.println("consulta " + consulta);
    Statement st = con.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
    ResultSet rs = st.executeQuery(consulta);
    if(rs.next()){
        loteMaterial = rs.getDouble("cantidad");
    }
    }catch(Exception e){
        e.printStackTrace();
    }
...
try {
        ...
        String conProductosEnEstabilidad  = request.getParameter("conProductosEnEstabilidad");
        ...
        String codigoProductoEstabilidad = "";
        if(conProductosEnEstabilidad.equals("1")){
            String consulta = " select cast(ppr.COD_PROGRAMA_PROD as varchar)+''+cast(f.COD_COMPPROD as varchar)+''+cast( p.COD_TIPO_PROGRAMA_PROD as varchar)+''+cast(p.COD_LOTE_PRODUCCION as varchar)" +
                    " from PROGRAMA_PRODUCCION_CONTROL_CALIDAD p" +
                    " inner join PROGRAMA_PRODUCCION_CONTROL_CALIDAD_ANALISIS pa on pa.COD_PROGRAMA_PROD_CONTROL_CALIDAD = p.COD_PROGRAMA_PROD_CONTROL_CALIDAD" +
                    " inner join FORMULA_MAESTRA f on f.COD_FORMULA_MAESTRA = p.COD_FORMULA_MAESTR" +
                    " inner join programa_produccion ppr on ppr.COD_COMPPROD = f.COD_COMPPROD and ppr.COD_LOTE_PRODUCCION = p.COD_LOTE_PRODUCCION" +
                    " and ppr.COD_TIPO_PROGRAMA_PROD = p.COD_TIPO_PROGRAMA_PROD" +
                    " inner join PROGRAMA_PRODUCCION_PERIODO prp on prp.COD_PROGRAMA_PROD = ppr.COD_PROGRAMA_PROD" +
                    " where pa.FECHA_ANALISIS between prp.FECHA_INICIO and prp.FECHA_FINAL and ppr.cod_programa_prod = '"+codProgramaProd+"'";
            System.out.println("consulta " + consulta);
            Statement st = con.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
            ResultSet rs = st.executeQuery(consulta);
            while(rs.next()){
                codigoProductoEstabilidad = codigoProductoEstabilidad+",'"+rs.getString("codigo_producto")+"'";
            }
        }
            ...
            sql4 = " select  m.COD_MATERIAL,m.NOMBRE_MATERIAL " +
                   " from MATERIALES m where m.MOVIMIENTO_ITEM = 1 and m.COD_MATERIAL in ( " +
            ...
            if(conProductosEnEstabilidad.equals("1")){
                sql4 += " union select distinct m.COD_MATERIAL,m.NOMBRE_MATERIAL" +
                        " from PROGRAMA_PRODUCCION_CONTROL_CALIDAD p" +
                        " inner join PROGRAMA_PRODUCCION_CONTROL_CALIDAD_ANALISIS pa on pa.COD_PROGRAMA_PROD_CONTROL_CALIDAD = p.COD_PROGRAMA_PROD_CONTROL_CALIDAD" +
                        ...
                        " and ppr.COD_TIPO_PROGRAMA_PROD = p.COD_TIPO_PROGRAMA_PROD" +
                        " inner join PROGRAMA_PRODUCCION_PERIODO prp on prp.COD_PROGRAMA_PROD = ppr.COD_PROGRAMA_PROD" +
                        " inner join PROGRAMA_PROD_CONTROL_CALIDAD_DETALLE pcd on pcd.cod_control_calidad_analisis = pa.COD_CONTROL_CALIDAD_ANALISIS" +
                        " inner join MATERIALES m on m.COD_MATERIAL = pcd.cod_material" +
                        " where pa.FECHA_ANALISIS between prp.FECHA_INICIO and prp.FECHA_FINAL and ppr.cod_programa_prod = '"+codProgramaProd+"' ";
            }
            ...
            sql_exp += " (select SUM(iade.cantidad_restante)" +
                    ...
                    " and iade.cod_material = iad.cod_material and iade.cod_ingreso_almacen = iad.cod_ingreso_almacen " +
                    " and ia.fecha_ingreso_almacen <= '" +fecha_existencia+" "+hora+"' " +
                    " and iade.cod_estado_material in (5) and iade.cantidad_restante > 0 and ia.cod_almacen in(1,2,9))as reanalisis,c.cod_capitulo ";
            sql_exp += " from materiales m,grupos g,capitulos c,UNIDADES_MEDIDA u ";
            sql_exp += " where m.cod_grupo=g.cod_grupo and g.cod_capitulo=c.cod_capitulo and  m.material_almacen=1 and m.movimiento_item = 1 and m.cod_estado_registro = 1 and u.cod_unidad_medida=m.cod_unidad_medida ";
            sql_exp += " and m.cod_material in ( " + items  + " ) ";
            sql_exp += " order by m.nombre_material";
            ...
            <DIV ALIGN="CENTER" CLASS="outputText2" ID="panel">
                ...
                <table width="95%" align="center" class="outputText2" style="border : solid #f2f2f2 1px;" cellpadding="0" cellspacing="0">
                    <tr class="headerClassACliente">
                        ...
                        <th  align="center" style="border : solid #f2f2f2 1px;">Tam. Lote</th>
                ...
                while (rs2.next()) {
                    ...
                    int codCapitulo = rs2.getInt("cod_capitulo");
                    double tamLote = 0.0;
                    if(codCapitulo==4 || codCapitulo == 8){
                        tamLote = this.loteMaterial(codMaterial);
                    }
                    ...
                    sql4 = " select sum(cantidad) cantidad from (SELECT ppd.CANTIDAD FROM PROGRAMA_PRODUCCION p, COMPONENTES_PROD cp," +
                            ...
                            "      fmes.cod_formula_maestra = p.cod_formula_maestra and      fmes.cod_material = ppd.cod_material and ppd.COD_MATERIAL in (" +
                                     ...
                                     " union all select fmdes.COD_MATERIAL from FORMULA_MAESTRA_DETALLE_ES fmdes where fmdes.COD_FORMULA_MAESTRA = p.cod_formula_maestra )";
                    if(conProductosEnEstabilidad.equals("1")){
                           sql4+=  " select  pcd.cantidad " +
                                   " from PROGRAMA_PRODUCCION_CONTROL_CALIDAD p" +
                                   " inner join PROGRAMA_PRODUCCION_CONTROL_CALIDAD_ANALISIS pa on pa.COD_PROGRAMA_PROD_CONTROL_CALIDAD = p.COD_PROGRAMA_PROD_CONTROL_CALIDAD" +
                                   " inner join FORMULA_MAESTRA f on f.COD_FORMULA_MAESTRA = p.COD_FORMULA_MAESTRA" +
                                   " inner join programa_produccion ppr on ppr.COD_COMPPROD = f.COD_COMPPROD and ppr.COD_LOTE_PRODUCCION = p.COD_LOTE_PRODUCCION" +
                                   " and ppr.COD_TIPO_PROGRAMA_PROD = p.COD_TIPO_PROGRAMA_PROD" +
                                   " inner join PROGRAMA_PRODUCCION_PERIODO prp on pa.FECHA_ANALISIS between prp.FECHA_INICIO and prp.FECHA_FINAL" +
                                   " inner join PROGRAMA_PROD_CONTROL_CALIDAD_DETALLE pcd on pcd.cod_control_calidad_analisis = pa.COD_CONTROL_CALIDAD_ANALISIS" +
                                   " inner join MATERIALES m on m.COD_MATERIAL = pcd.cod_material" +
                                   " where ppr.cod_programa_prod = '"+codProgramaProd+"' and pcd.cod_material = '"+codMaterial+"' ";
                          }
                          sql4+="  ) as tabla ";
                    ...
                    <tr >
                        ...
                        <td   style="border : solid #f2f2f2 1px;"><%=tamLote%></td>
                        ...
                        try {
                            ...
                            sqlCompra = " SELECT  cp.nombre_prod_semiterminado, sum(ppd.CANTIDAD) CANTIDAD,count(p.CANT_LOTE_PRODUCCION) cant,tpp.nombre_tipo_programa_prod,t.nombre_tipo_material,4 cod_estado_programa_prod" +
                                     ...
                                     " group by cp.nombre_prod_semiterminado,tpp.nombre_tipo_programa_prod,t.nombre_tipo_material ";
                            if(conProductosEnEstabilidad.equals("1")){
                                sqlCompra +=" union all  select cp.nombre_prod_semiterminado,sum(pcd.cantidad) cantidad,count(ppr.COD_LOTE_PRODUCCION) cant,tp.NOMBRE_TIPO_PROGRAMA_PROD,'' nombre_tipo_material,20 cod_estado_programa_prod" +
                                         " from PROGRAMA_PRODUCCION_CONTROL_CALIDAD p" +
                                         " inner join PROGRAMA_PRODUCCION_CONTROL_CALIDAD_ANALISIS pa on pa.COD_PROGRAMA_PROD_CONTROL_CALIDAD = p.COD_PROGRAMA_PROD_CONTROL_CALIDAD" +
                                         " inner join FORMULA_MAESTRA f on f.COD_FORMULA_MAESTRA = p.COD_FORMULA_MAESTRA" +
                                         ...
                                         " inner join PROGRAMA_PRODUCCION_PERIODO prp on pa.FECHA_ANALISIS between prp.FECHA_INICIO and prp.FECHA_FINAL" +
                                         " inner join PROGRAMA_PROD_CONTROL_CALIDAD_DETALLE pcd on pcd.cod_control_calidad_analisis = pa.COD_CONTROL_CALIDAD_ANALISIS" +
                                         " inner join TIPOS_PROGRAMA_PRODUCCION tp on tp.COD_TIPO_PROGRAMA_PROD = ppr.COD_TIPO_PROGRAMA_PRO" +
                                         " inner join MATERIALES m on m.COD_MATERIAL = pcd.cod_material" +
                                         " where  ppr.cod_programa_prod = '"+codProgramaProd+"'" +
                                         " group by cp.nombre_prod_semiterminado,tp.nombre_tipo_programa_prod ";
                            }
                            ...
                            while (rsCompra.next()) {
                                ...
                                %>
                                <tr style="<%=codEstadoProgramaProd.equals("7")?"background-color:aqua":""%><%=codEstadoProgramaProd.equals("20")?"background-color:blue":""%>">
