        ...
        <head>
            ...
            <script  type="text/javascript">
                ...
                function getCodigo(codigo,codFormula,cantidad,codComProd,codLote,codTipoProgramaProd){
                    ...
                    url='detalle_navegador_programa_prod_desarrollo.jsf?codigo='+codigo+'&codFormula='+codFormula+'&cantidad='+cantidad+'&cod_comp_prod='+codComProd+'&cod_lote='+codLote+'&codTipoProgramaProd='+codTipoProgramaProd;
                    //alert(url);
            ...
            function verReporteMateriales(codProgramaProd,codformula,codComprod,codtipoProg,codlote){
                 window.open('reporteCotizacionesVentasDemandaClientes.jsf?codComprod='+codComprod+'&codProgProd='+codProgramaProd+
                 '&codForm='+codformula+'&codTipoProg='+codtipoProg+'&codLote='+codlote,'DETALLE','top=50,left=200,width=800,height=600,scrollbars=1,resizable=1');
            }
            function asignarCantidadIngresoAcond(celda)
            {
                var tablaClientes=document.getElementById('form2:programaProduccionClientes');
                var cantidadRequerida=0;
              
                for(var i=1;i<tablaClientes.rows.length;i++)
                {
                    if(parseInt(celda.parentNode.parentNode.rowIndex)!=parseInt(i))
                        {
                            if(tablaClientes.rows[i].cells[0].getElementsByTagName('input')[0]!=null)
                            {
                                tablaClientes.rows[i].cells[0].getElementsByTagName('input')[0].checked=false;
                            }
                        }

                }
                celda.checked=true;
                cantidadRequerida=parseFloat(celda.parentNode.parentNode.cells[5].getElementsByTagName('span')[0].innerHTML);
                var tablaPrograma=document.getElementById("form2:detalle");
                
                
                tablaPrograma.rows[1].cells[5].getElementsByTagName('input')[0].value=cantidadRequerida;
                
            }
            function validarRegistroCantidades()
            {
                ...
                {
                    if(tablaClientes.rows.length>1)
                    {
                        for(var i=1;i<tablaClientes.rows.length;i++)
                        {
                            if(tablaClientes.rows[i].cells[0].getElementsByTagName('input')[0]!=null)
                            {
                                if(tablaClientes.rows[i].cells[0].getElementsByTagName('input')[0].checked==true)
                                    {
                                        cantidadPermitida=parseFloat(tablaClientes.rows[i].cells[5].getElementsByTagName('span')[0].innerHTML);
                                    }
                            }
                        }
                        var tablaPrograma=document.getElementById("form2:detalle");
                        ...
                        cantidadRegistrada=parseFloat(tablaPrograma.rows[1].cells[5].getElementsByTagName('input')[0].value);
                        if(cantidadPermitida>0)
                            {
                                if(cantidadRegistrada>cantidadPermitida)
                                    {
                                        alert('No se puede registrar una cantidad Mayor a la permitida para el cliente');
                                        return false;
                                    }
                                    else
                                    {
        ...
        <%--<body onLoad="window.defaultStatus='Hola, yo soy la barra de estado.';">--%>
            ...
            <a4j:form id="form1">
                <div align="center">
                    ...
                    <h:outputText value="#{ManagedProgramaProduccionDesarrollo.cargarProgramaProduccion1}"  />
                    <h:outputText styleClass="outputTextTitulo"  value="Programas de Producción" />
                    <br><br>
                         Estado Programa Producción:
                         <h:selectOneMenu  styleClass="inputText" value="#{ManagedProgramaProduccionDesarrollo.codEstadoProgramaProduccion}"
                                          id="codFormulaMaestra">
                                              <f:selectItems value="#{ManagedProgramaProduccionDesarrollo.estadosProgramaProduccion}"/>
                                              <a4j:support event="onchange" action="#{ManagedProgramaProduccionDesarrollo.estadoProgramaProduccion_Change}"
                    ...
                    <br> <br>
                        ...
                        <rich:dataTable value="#{ManagedProgramaProduccionDesarrollo.programaProduccionList}" var="data" id="dataFormula"
                                    onRowMouseOut="this.style.backgroundColor='#FFFFFF';"
                                    onRowMouseOver="this.style.backgroundColor='#DDE3E4';"
                                    headerClass="headerClassACliente" binding="#{ManagedProgramaProduccionDesarrollo.programaProduccionDataTable}">
                        ...
                        <rich:column>
                            <f:facet name="header">
                                <h:outputText value="Pesaje"  />
                            </f:facet>
                            <h:commandLink action="#{ManagedProgramaProduccionDesarrollo.verActividadesProduccion_action}" >
                                <f:param name="codAreaEmpresa" value="97" />
                                <f:param name="url" value="../programaProduccionDesarrollo/navegador_programa_produccion_desarrollo.jsf" />
                                <h:graphicImage url="../img/organigrama3.jpg" alt="proceso de pesaje" />
                            </h:commandLink>
                        </rich:column>
                        <rich:column>
                            <f:facet name="header">
                        ...
                        <rich:column>
                            ...
                            <a4j:commandLink action="#{ManagedProgramaProduccionDesarrollo.verReporteEtiquetas_action}" oncomplete="openPopup('reporteEtiquetas.jsp')">
                        ...
                        <rich:column  >
                            <f:facet name="header">
                             ...
                             <a4j:commandLink styleClass="outputText2" action="#{ManagedProgramaProduccion.agregarControlDeCalidad}" oncomplete="if('#{ManagedProgramaProduccion.mensaje}'!=''){if(confirm('#{ManagedProgramaProduccion.mensaje}')==true){showPage();}}else{showPage();}">
                                 <h:graphicImage url="../img/detalle.jpg" title="Analisis de Control de Calidad"/>
                             </a4j:commandLink>
                        </rich:column--%>
                    </rich:dataTable>
                    <br>                        
                    <h:commandButton value="Agregar" action="#{ManagedProgramaProduccionDesarrollo.agregarProgramaProduccionDesarrollo_action}"  styleClass="btn"  /> 
                    <h:commandButton value="Editar Lote"    styleClass="btn"  action="#{ManagedProgramaProduccionDesarrollo.editarProgramaProduccion_action}" onclick="return editarItem('form1:dataFormula');"/>
                    <a4j:commandButton value="Eliminar"styleClass="btn"  action="#{ManagedProgramaProduccionDesarrollo.eliminarProgProd}" oncomplete="alert('Se elimino el programa de producción');" reRender="dataFormula" onclick="if(confirm('Esta seguro de eliminar el registro?')==false){return false}else{if(eliminar('form1:dataFormula')==false){return false;}}"  />
                    </h:panelGroup>
                </div>
