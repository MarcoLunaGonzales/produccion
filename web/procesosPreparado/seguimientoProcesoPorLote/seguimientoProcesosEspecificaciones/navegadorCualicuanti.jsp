            ...
            <table width="80%" align="center" class="outputText0" style="border : solid #000000 1px;" cellpadding="0" cellspacing="0" >
                ...
                int vidaUtil=0;
                  try
                  {
                    ...
                    String consulta="select pp.COD_FORMULA_MAESTRA_VERSION,pp.cod_formula_maestra, isnull(cpr.COD_RECETA_LAVADO, 0) as cod_receta_lavado,f.cod_forma,pp.COD_FORMULA_MAESTRA,cp.COD_COMPPROD,f.abreviatura_forma,p.nombre_prod,cp.NOMBRE_GENERICO,f.nombre_forma,cp.REG_SANITARIO,"+
                    ...
                    int codFormulaMaestraVersion=0;
                    if(res.next())
                    {
                        codFormulaMaestraVersion=res.getInt("COD_FORMULA_MAESTRA_VERSION");
                       ...
                       //verifificar que todos los materiales esten registrados en el preparado
                       int nroDiagrama=0;
                       if(codComprodMix.split(",").length>1)
                       {
                           consulta="select cp.NRO_COMPPROD_DIAGRAMA_PREPARADO"+
                                    " from COMPONENTES_PROD_MIX cp where cp.COD_COMPROD1='"+(prod1[0].intValue())+"' and cp.COD_COMPROD2='"+(prod2[0].intValue())+"'";
                           System.out.println("consulta buscar mix"+consulta);
                           res=st.executeQuery(consulta);
                           if(res.next())nroDiagrama=res.getInt("NRO_COMPPROD_DIAGRAMA_PREPARADO");
                           if(nroDiagrama==0)out.println("<script>alert('No se ha definido los productos del mix. No se puede evaluar la preparación del producto');</script>");
                       }
                       else {
                           nroDiagrama=1;
                       }
                       consulta="select m.NOMBRE_MATERIAL "+
                                    " from FORMULA_MAESTRA_DETALLE_MP_FRACCIONES fmd inner join MATERIALES m on fmd.COD_MATERIAL=m.COD_MATERIAL"+
                                    " where  fmd.COD_FORMULA_MAESTRA_FRACCIONES  not in ("+
                                    " select pp.COD_FORMULA_MAESTRA_FRACCIONES"+
                                    " from PROCESOS_PRODUCTO_ESP_MAT pp where pp.COD_FORMULA_MAESTRA=fmd.COD_FORMULA_MAESTRA"+
                                    " and pp.COD_MATERIAL=fmd.COD_MATERIAL and pp.PROCESO_TRANSITORIO=0"+
                                    " union select spp.COD_FORMULA_MAESTRA_FRACCIONES from SUB_PROCESOS_PRODUCTO_ESP_MAT spp where spp.COD_FORMULA_MAESTRA=fmd.COD_FORMULA_MAESTRA"+
                                    " and spp.COD_MATERIAL=fmd.cod_material and spp.PROCESO_TRANSITORIO=0)"+
                                    " and fmd.COD_FORMULA_MAESTRA="+(nroDiagrama==1?prod1[2].intValue():prod2[2].intValue())+
                                    " group by m.NOMBRE_MATERIAL" +
                                    " order by m.NOMBRE_MATERIAL";
                           System.out.println("consulta verificar preparado "+consulta);
                           res=st.executeQuery(consulta);
                      String materialesSinAsociar="";
                      while(res.next())
                      {
                          materialesSinAsociar+=(materialesSinAsociar.equals("")?"":",")+res.getString("NOMBRE_MATERIAL");
                      }
                      if(!materialesSinAsociar.equals(""))out.println("<script>alert('No se han asociado todos los materiales al preparado:"+materialesSinAsociar+"');</script>");
                       consulta="select sum(isnull(procesos.noConsolidados,0)) as contProcesos,sum(isnull(subProcesos.subNoConsolidados,0)) as contSubProcesos"+
                                " from PROCESOS_PRODUCTO p "+
                                " outer apply(select count(*) as noConsolidados from PROCESOS_PRODUCTO_ESP_EQUIP ppee where"+
                                " ppee.COD_PROCESO_PRODUCTO=p.COD_PROCESO_PRODUCTO"+
                                " and ppee.DATOS_NO_CONSOLIDADOS=1) procesos"+
                                " outer APPLY( select count(*)   as subNoConsolidados"+
                                " from SUB_PROCESOS_PRODUCTO_ESP_EQUIP sppe where sppe.COD_PROCESO_PRODUCTO=p.COD_PROCESO_PRODUCTO"+
                                " and sppe.DATOS_NO_CONSOLIDADOS=1) subProcesos"+
                                " where p.COD_COMPPROD='"+(nroDiagrama==1?prod1[0].intValue():prod2[0].intValue())+"'group by p.COD_COMPPROD";
                       System.out.println("consulta verificar datos no consolidados "+consulta);
                       res=st.executeQuery(consulta);
                       if(res.next())
                       {
                           if(res.getInt("contProcesos")>0 ||res.getInt("contSubProcesos")>0)
                           {
                               out.println("<script>alert('Existen especificaciones de maquinarias aun no consolidadas');</script>");
                           }
                       }
                       ...
                       // System.out.println(volumenfabricar);
                            ...
                            consulta="select fmd.CANTIDAD_UNITARIA,m.COD_MATERIAL,m.NOMBRE_MATERIAL,um.ABREVIATURA,sum(fmdf.CANTIDAD) as CANTIDAD,um.COD_UNIDAD_MEDIDA"
                                    + " ,fmd.NRO_DECIMALES_ALMACEN"+
                                     " from FORMULA_MAESTRA_DETALLE_MP_VERSION fmd " +
                                     "inner join FORMULA_MAESTRA_DETALLE_MP_FRACCIONES_VERSION fmdf"+
                                     " on fmd.COD_FORMULA_MAESTRA=fmdf.COD_FORMULA_MAESTRA and "+
                                     " fmd.COD_MATERIAL=fmdf.COD_MATERIAL and fmd.COD_VERSION=fmdf.COD_VERSION"+
                                    ...
                                    + " and fmd.COD_VERSION='"+codFormulaMaestraVersion+"'"
                                    + " and isnull(fmdf.COD_TIPO_MATERIAL_PRODUCCION,0) =0" +
                                     " group by  fmd.CANTIDAD_UNITARIA,fmd.NRO_DECIMALES_ALMACEN,m.COD_MATERIAL,m.NOMBRE_MATERIAL,um.ABREVIATURA,um.COD_UNIDAD_MEDIDA" +
                                     " order by m.NOMBRE_MATERIAL";
                            ...
                            while(res.next())
                            {
                                ...
                                cantidadLote=res.getDouble("CANTIDAD");
                                cantidadLote=(res.getInt("COD_UNIDAD_MEDIDA")==3?1000:1)*cantidadLote;
                                cantidadUnitaria=res.getDouble("CANTIDAD_UNITARIA");
                                cantidadUnitaria=(res.getInt("COD_UNIDAD_MEDIDA")==3?1000:1)*cantidadUnitaria;
                                cantidadLote=((tamanoLote/prod1[3])*cantidadLote);//tamaño total del lote(mix,adjunto)divido entre el tamaño del lote de la formula
                                cantidadLote=((Math.rint(cantidadLote*(Math.pow(10,res.getDouble("NRO_DECIMALES_ALMACEN")))))/Math.pow(10d, res.getDouble("NRO_DECIMALES_ALMACEN")));
                                abreviatura=(res.getInt("COD_UNIDAD_MEDIDA")==3?"g":res.getString("ABREVIATURA"));
                                
                                        sumaTotal+=cantidadLote;
