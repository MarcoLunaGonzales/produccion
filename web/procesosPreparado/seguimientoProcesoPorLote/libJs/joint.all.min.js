 ...
 */Raphael=function(){
    ...
    var t=D.Joint;D=t.dia.Element;var F=t.point,g=t.dia.uml={};t.arrows.aggregation=function(){
        ...
        calcular:function(v){
			...
			if((v.rect.y+v.rect.height)>((contPro*tamHoja)+((contPro-1)*heightCabecera)-80))
			{
				if(parseInt(v.codSubProceso)>0)
				{
				...
				puntos[puntos.length]=new puntoInterupcion(((contPro*tamHoja)+((contPro-1)*heightCabecera)),listaLetras[contAsignador]);
				//this.paper.circle(10,((contPro*tamHoja)+((contPro-1)*heightCabecera)+80),1);
				//this.paper.circle(10,((contPro*tamHoja)+((contPro-1)*heightCabecera)-80),1);
        ...
        getMateriales:function(g){
            ...
            var i=this.properties;
                ...
                i=this.properties;
                var romp=0;
                while(romp<i.materiales.length)
                {
                    var k='';
                    i=this.properties;
                         for(var y=romp;(y<i.materiales.length&&y<(romp+8));y+=2)
                            k+='-'+i.materiales[y]+": "+i.materiales[y+1]+"\n";
                        k=k.replace(/^\s\s*/,"").replace(/\s\s*$/,"");
                        s=this.wrapper.getBBox();
                        pisco=this.paper.text(s.x+i.actionsOffsetX,s.y+i.labelOffsetY+i.labelHeight+i.actionsOffsetY+n.coorYText,k);
                        k=pisco.getBBox();pisco.attr(n.textNormal);pisco.translate(0,k.height/2);
                        n.coorYText+=k.height;
                        g.addInner(pisco);
                    romp+=8;
                }
