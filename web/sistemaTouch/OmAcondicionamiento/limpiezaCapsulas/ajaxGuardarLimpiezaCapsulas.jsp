...
//datos actividad
int codActividadLimpiezaCapsulas=Integer.valueOf(request.getParameter("codActividadLimpiezaCapsulas"));
String[] dataLimpiezaCapsulas=request.getParameter("dataLimpiezaCapsulas").split(",");
try
{
        con=Util.openConnection(con);
        con.setAutoCommit(false);
        StringBuilder consulta=new StringBuilder("select MAX(s.COD_SEGUIMIENTO_LIMPIEZA_CAPSULAS_ACOND) as codSeguimiento");
                                consulta.append(" from SEGUIMIENTO_LIMPIEZA_CAPSULAS_ACOND s ");
        ...
        if(codSeguimiento==0)
        {
            consulta=new StringBuilder("INSERT INTO SEGUIMIENTO_LIMPIEZA_CAPSULAS_ACOND(COD_LOTE,COD_PROGRAMA_PROD,  COD_ESTADO_HOJA , COD_COMPPROD, COD_TIPO_PROGRAMA_PROD");
        ...
        else
        {
                if(codTipoPermiso==12&&codTipoGuardado==2)
                {
                    consulta=new StringBuilder("UPDATE SEGUIMIENTO_LIMPIEZA_CAPSULAS_ACOND");
                            ...
                            consulta.append(" WHERE COD_SEGUIMIENTO_LIMPIEZA_CAPSULAS_ACOND =").append(codSeguimiento);
        ...
        consulta=new StringBuilder("delete SEGUIMIENTO_PROGRAMA_PRODUCCION_PERSONAL");
                    consulta.append(" where COD_PROGRAMA_PROD=").append(codprogramaProd);
                            ...
                            consulta.append(" and COD_ACTIVIDAD_PROGRAMA in (").append(codActividadLimpiezaCapsulas).append(")");
        ...
        consulta=new StringBuilder("INSERT INTO SEGUIMIENTO_PROGRAMA_PRODUCCION_PERSONAL(COD_COMPPROD,COD_PROGRAMA_PROD,COD_LOTE_PRODUCCION,COD_FORMULA_MAESTRA,");
                        ...
                        consulta.append(" VALUES (");
                                ...
                                consulta.append(codActividadLimpiezaCapsulas).append(",");
                                ...
                                consulta.append("0,");//unidades extra
                                consulta.append("?");//cod registro om
                        consulta.append(")");
        System.out.println("consulta registro tiempo limpieza capsulas "+consulta.toString());
        pst=con.prepareStatement(consulta.toString());

        for(int i=0;(i<dataLimpiezaCapsulas.length&&dataLimpiezaCapsulas.length>1);i+=6)
        {
                String[] aux=dataLimpiezaCapsulas[i+1].split("/");
                fechaInicio=aux[2]+"/"+aux[1]+"/"+aux[0]+" "+dataLimpiezaCapsulas[i+2];
                fechaFinal=aux[2]+"/"+aux[1]+"/"+aux[0]+" "+dataLimpiezaCapsulas[i+3];

                pst.setString(1,dataLimpiezaCapsulas[i]);System.out.println("p1:"+dataLimpiezaCapsulas[i]);
                pst.setString(2,dataLimpiezaCapsulas[i+4]);System.out.println("p2:"+dataLimpiezaCapsulas[i+4]);
                pst.setString(3,dataLimpiezaCapsulas[i+5]);System.out.println("p3:"+dataLimpiezaCapsulas[i+5]);
                pst.setString(4,fechaInicio);System.out.println("p4:"+fechaInicio);
                pst.setString(5,fechaFinal);System.out.println("p5:"+fechaFinal);
                pst.setInt(6,i);System.out.println("p6:"+i);
                
                if(pst.executeUpdate()>0)System.out.println("se registro el seguimiento Personal limpieza capsulas");
        }
            ...
            pst.setInt(5,codActividadLimpiezaCapsulas);System.out.println("p5:"+codActividadLimpiezaCapsulas);
